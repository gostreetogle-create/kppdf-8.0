...
- 2026-08-22 TZD-60 (desktop installer errors hardening) - DEFERRED by claude-computer; STEP 1 (verbatim dialogs from a live clean Windows install) not possible in this session: non-interactive PowerShell only, no screen/GUI input, NSIS silent mode hides dialogs, and STEP 1 needs uninstall on PO machine; zero code diff, no lock; archive tasks/_archive/2026-08/TZD-60.deferred.md.
- 2026-08-25 TZ-DESK-440 (tray primary CTA — live actions only) - IN PROGRESS by freebuff (Buffy); commit e835003b (pushed main); gates: FE tsc PASS, tray spec 20/20 PASS, desk page spec 37/37 PASS, eslint 0 errors (17 pre-existing warnings), architecture:check FAIL only on foreign violations (materials/supply = other TZs, my files 0); archive pending Cursor/PO live /desk PASS (review handoff).
