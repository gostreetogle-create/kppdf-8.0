...
3. **Dialog demos (TZ-73):** `PiDialogService.open()` сигнатура `(component, config)` — для demo используется toast. Полная CDK-overlay flow: `dialog.open(PiDialogComponent, { data: {...} })`. **Stabilization DOC-340 / UX-DIALOG-301:** shell widths `min(…px, calc(100vw - 2rem))`; wide form dialogs clamp the same way. Canon: `docs/DIALOG-COOKBOOK.md`.
