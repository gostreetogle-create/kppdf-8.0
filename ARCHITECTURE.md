...
| Frontend layout | `frontend/src/app/layout/` | AppLayout (operational), KitLayout (UI showcase). **TZ-UX-304/307:** top nav L→R = Каталог…Документы → Справочники → Админ; пункт = rect (~h-10) + icon 12px + `shortLabel` (полный RU в aria/title); header ~h-14; equal-width от коротких. Stubs `/counterparties` `/design` `/supply` `/shipping`. |
