...
3. **Dialog demos (TZ-73):** `PiDialogService.open()` сигнатура `(component, config)` — для demo используется toast. Полная CDK-overlay flow: `dialog.open(PiDialogComponent, { data: {...} })`. **Stabilization DOC-340 / UX-DIALOG-301 / UX-DIALOG-302:** shell widths `min(…px, calc(100vw - 2rem))`; form xl ~920; QuickCreate S/M/L→md/lg/xl + 2-col. Canon kinds A–D: `docs/DIALOG-COOKBOOK.md` + `docs/pages/ui-dialog-canon.md`.
