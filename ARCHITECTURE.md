...
| Frontend layout | `frontend/src/app/layout/` | AppLayout (operational), KitLayout (UI showcase). **TZ-UX-304:** top nav L→R = Каталог…Документы → Справочники → Админ; пункт = rect + icon + caption (`shortLabel`). Stubs `/counterparties` `/design` `/supply` `/shipping`. |
