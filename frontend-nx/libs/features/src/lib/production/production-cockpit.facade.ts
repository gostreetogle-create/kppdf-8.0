/**
 * TZ-NX-PRODUCTION-COCKPIT-FACADE — write/UI facade for `ProductionCockpitPage`.
 *
 * Owns: order/bar state, range/zoom-adjacent signals, optimistic Gantt writes
 * (estimate days / planned date / start offset / worker assignment / order
 * meta), left-tool flyout state, and initial-load + refresh orchestration —
 * moved as-is from the page. `ProductionReadFacade` stays a separate
 * read-only model (not merged here); `ProductionCockpitContext` stays the
...
import type {
  GanttEstimateDaysCommit,
  GanttOrderMetaCommit,
  GanttOrderMetaView,
...
@Injectable()
export class ProductionCockpitFacade {
  private readonly ctx = inject(ProductionCockpitContext);
  ...
  readonly unassignedGanttWork = computed(() => summarizeUnassignedGanttWork(this.bars()));
  readonly rangeStart = signal(defaultRangeStart());
  readonly rangeEnd = signal(defaultRangeEnd());
  readonly usedTodayFallback = signal(false);
  readonly readOnly = signal(false);
  /** TZ-GANTT-401 — row grouping mode (По заказам | По рабочим). */
  readonly groupBy = signal<GanttGroupBy>('orders');
  readonly workerLabels = signal<ReadonlyMap<string, string>>(new Map());
  readonly workerCandidates = signal<ReadonlyMap<string, readonly Person[]>>(new Map());
  readonly workerAssignmentSaving = signal(false);
  readonly orderThumbs = signal<
    ReadonlyMap<
      string,
  ...
  readonly orderIdHint = signal<string | null>(null);
  /** Deep-link return — visible only when arriving from the desk. */
  private readonly fromDesk = signal(false);
  readonly returnOrderId = signal<string | null>(null);
  readonly returnLink = computed<Record<string, string> | null>(() =>
  ...
  /** Left-tool flyout state; the page's ShellToolRail buttons call `toggleLeftTool`. */
  readonly leftTool = signal<ProductionLeftTool>(null);
  ...
  readonly canEditCatalog = computed(() => this.caps.hasAny(['production:write']));

  /** When order-meta strip is open — highlight that order on the Gantt. */
  readonly metaHighlightOrderId = computed(() => {
  ...
  readonly noGanttOrderIds = computed(
    () => new Set(this.facade.state().ineligible.map((row) => row.orderId)),
  );

  readonly orderMetaView = computed((): GanttOrderMetaView | null => {
    if (!this.ctx.orderMetaOpen()) return null;
  ...
  onToggleExpand(expandId: string): void {
  ...
  onToggleWorkDetail(barId: string): void {
   ...
   */
  async onOrderLabelClick(id: string): Promise<void> {
  ...
  toggleLeftTool(tool: Exclude<ProductionLeftTool, null>, event?: Event): void {
  ...
  async onWorkerAssignmentCommit(ev: GanttWorkerAssignmentCommit): Promise<void> {
  ...
  async onOrderMetaCommit(ev: GanttOrderMetaCommit): Promise<void> {
  ...
  async onEstimateDaysCommit(ev: GanttEstimateDaysCommit): Promise<void> {
  ...
  async onPlannedDateMoveCommit(ev: GanttPlannedDateMoveCommit): Promise<void> {
    if (!this.canEditOrder()) return;
  ...
  async onStartOffsetCommit(ev: GanttStartOffsetCommit): Promise<void> {
    if (!this.canEditCatalog()) return;
    const deltaDays = Math.trunc(ev.deltaDays);
    if (deltaDays === 0) return;
    const order = this.orders().find((o) => o._id === ev.orderId);
  ...
  async onFitHorizon(): Promise<void> {
  ...
  private requestTimelineScroll(target: 'today' | 'start' | 'bar', barId?: string): void {
    this.scrollRequest.set(
      barId ? { target, nonce: ++this.scrollNonce, barId } : { target, nonce: ++this.scrollNonce },
    );
  }
  ...
  handleBarsAfterShift(bars: GanttBar[], orderId: string): void {
    this.bars.set(bars);
    this.refitRangeAfterShift(bars, orderId);
  }

  /** Initial load (deep-link aware). Called from the constructor. */
