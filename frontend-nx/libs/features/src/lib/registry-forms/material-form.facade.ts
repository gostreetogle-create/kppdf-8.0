/**
 * TZ-NX-REGISTRY-FORMS-FACADE — domain facade for `MaterialFormDialogComponent`.
 *
 * Owns: form/catalog/photo/dimension signals and every load/submit/payload
 ...
 * `RoleFormFacade`). `@ViewChild('formEl')` stays on the component (only it
 * can resolve against its own template); the one DOM-focus side effect from
 * `onSubmit()` is bridged via an optional callback parameter.
 */
...
export type DimensionGroup = FormGroup<{
...
@Injectable()
export class MaterialFormFacade {
  readonly dimensionTypes = DIMENSION_TYPES;
  readonly formatKind = formatMaterialKind;
  ...
  readonly data = inject<MaterialFormDialogData>(PI_DIALOG_DATA);
  private readonly ref = inject<DialogRef<Material | null | undefined>>(PI_DIALOG_REF);
  readonly dialog = inject(PiDialogService);
  ...
  readonly isDetailForm = computed(
   ...
   */
  readonly categoryRequired = this.isDetailForm;
  ...
  readonly showKindSelect = computed(
  ...
  private buildInvalidMessage(): string {
    const missing = MaterialFormFacade.REQUIRED_FIELDS.filter(
  ...
  firstInvalidFieldHtmlId(): string | undefined {
    return MaterialFormFacade.REQUIRED_FIELDS.find((f) => this.form.controls[f.key].invalid)?.htmlId;
  }
