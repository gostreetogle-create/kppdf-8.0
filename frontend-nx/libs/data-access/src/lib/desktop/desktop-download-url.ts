...
/**
 * TZD-72 — NX port of `frontend/src/app/core/desktop-download-url.ts`
 * (legacy). Same-origin installer path used when deployment does not
 * configure a URL.
 *
 * Canon (TZD-46): the versioned file `kppdf-desktop-setup-v{semver}.zip` is
 * the canonical artifact; this unversioned alias is a stable fallback for
 * old bookmarks and for deployments without a meta override. Deployments
 * SHOULD inject the versioned URL via the `kppdf-desktop-download-url` meta
 * tag (`DESKTOP_DOWNLOAD_URL` in deploy config) — see TZD-71's
 * `frontend-nx/apps/kppdf-web/src/index.html` meta marker.
 */
export const DEFAULT_DESKTOP_DOWNLOAD_URL = '/downloads/kppdf-desktop-setup.zip';

/** Meta tag written by `apps/kppdf-web/src/index.html` / deploy hook (CSP-safe; no inline script). */
 ...
 * `<meta name="kppdf-desktop-download-url" content="…">` from
 * `DESKTOP_DOWNLOAD_URL` before the bundle runs. An explicitly empty
 * content disables the action; an absent content attribute uses the
 * same-origin default.
 */
