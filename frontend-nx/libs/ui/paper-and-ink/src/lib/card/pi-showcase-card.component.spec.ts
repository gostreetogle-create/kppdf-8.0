...
describe('PiShowcaseCardComponent — TZ-PRODUCTS-305', () => {
  ...
  it('TZ-UX-344: .sc-media img uses object-fit contain (md does not force cover)', () => {
    ...
    expect(css).toMatch(/\.sc-media\s+img\s*\{[^}]*object-fit:\s*contain/);
    expect(css).toMatch(/\.sc-media\s+img\s*\{[^}]*object-position:\s*center/);
    expect(css).not.toMatch(/\.sc-media--md\s+img\s*\{[^}]*object-fit:\s*cover/);
    expect(css).not.toMatch(/\.sc-media\s+img\s*\{[^}]*object-fit:\s*cover/);
  });
});
