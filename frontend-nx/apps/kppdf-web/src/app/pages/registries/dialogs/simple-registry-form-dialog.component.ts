...
export type SimpleRegistryDialogKind = 'organization' | 'passport';
...
export class SimpleRegistryFormDialogComponent {
  ...
  readonly fields: readonly { key: string; label: string; type?: PiInputType }[] = this.data.kind === 'organization' ? [{ key: 'name', label: 'Название' }, { key: 'inn', label: 'ИНН' }] : [{ key: 'passportNumber', label: 'Номер паспорта' }, { key: 'productId', label: 'Изделие' }];
