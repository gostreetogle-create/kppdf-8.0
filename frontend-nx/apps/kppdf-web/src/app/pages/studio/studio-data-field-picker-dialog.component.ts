...
export interface StudioDataFieldSelection {
  source: string;
  sourceLabel: string;
  field: RegistryDataField;
}

export interface StudioDataFieldPickerDialogData {
  sources: readonly RegistryDataSource[];
  columnIndex: number;
}

const GROUP_LABELS: Record<'contacts' | 'catalog' | 'work', string> = {
 ...
 */
@Component({
  selector: 'pi-studio-data-field-picker-dialog',
  standalone: true,
  ...
  template: `
    <app-pi-dialog
      ...
      [showClose]="true"
    >
      <div body class="dfpd-body" data-test="studio-data-field-picker">
        <div class="dfpd-hint">
                    ...
                    >
                      ...
                      <span class="dfpd-source-meta">
                        <span class="dfpd-source-count"
                          >{{ (src.fields?.length ?? 0) }} {{ fieldWord((src.fields?.length ?? 0)) }}</span
                        >
...
export class StudioDataFieldPickerDialogComponent {
  protected readonly data = inject<StudioDataFieldPickerDialogData>(PI_DIALOG_DATA);
  private readonly ref = inject<DialogRef<StudioDataFieldSelection | null>>(PI_DIALOG_REF);

  protected readonly selectedSource = signal<RegistryDataSource | null>(null);
  protected readonly pendingField = signal<RegistryDataField | null>(null);

  protected readonly groupedSources = computed(() => {
    const map = new Map<RegistryDataSource['group'], RegistryDataSource[]>();
    ...
    for (const other of contacts.filter(
      (s) => !(CONTACT_SOURCE_ORDER as readonly string[]).includes(s.key),
    )) {
      result.push({
  ...
  protected selectField(field: RegistryDataField): void {
