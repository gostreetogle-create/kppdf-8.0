...
closed_by: QA-валидатор / Frontend Component Engineer (Buffy)
protected_files:
  - frontend/src/app/pages/admin/reset-password-dialog.component.spec.ts (NEW)
  - frontend/src/app/pages/admin/user-form-dialog.component.spec.ts (NEW)
  - frontend/src/app/pages/admin/role-form-dialog.component.spec.ts (NEW)
affected_areas:
  - frontend/src/app/pages/admin/*-dialog.component.spec.ts (3 NEW additive files)
verification:
  - jest src/app/pages/admin: 23/23 PASS (old 5 + new 18)
  - tsc -p tsconfig.app.json --noEmit: exit 0
  - smoke tests instantiate dialogs via TestBed (NG5xxx regression guard)
  - git diff --check: PASS
  - code review: PASS (code-reviewer-deepseek-flash)
  - lock file: .mimocode/locks/TZ-264-admin-dialogs-unit-tests.lock
notes: Компоненты .ts НЕ изменялись (additive specs против финальных компонентов TZ-261/TZ-265).
------------------------------------------------------------
