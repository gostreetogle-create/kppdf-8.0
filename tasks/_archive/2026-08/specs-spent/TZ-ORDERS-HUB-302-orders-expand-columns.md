...
### VISUAL LOCK (PO-approved, 2026-08-15)

Этот визуальный контракт является частью DoD и не должен быть перезаписан при последующих TZ:

- внешняя панель expand — мягкая светло-жёлтая подложка `--color-sunrise-soft`;
- четыре внутренние группы — светлые paper-raised блоки, без насыщенной жёлтой заливки;
- группы фиксированы: **Заказ**, **Исполнение**, **Логистика**, **Документы**;
- между группами — тонкие нейтральные hairline-разделители и компактные заголовки;
- содержимое внутри каждой группы выровнено по верхнему краю и следует ритму «заголовок → данные → действие»;
- нельзя возвращать одну плоскую жёлтую сетку, усиливать рамки или разбрасывать действия по краям ячеек без отдельного PO-pass;
- функциональные блоки, ссылки, read-only границы и write-path не менять ради декоративной правки.

Любая следующая правка expanded-панели обязана сохранить эти четыре группы и пройти визуальный review на реальном `/orders`.
