...
GATES (все зелёные до archive; nx build — ПОСЛЕДНИМ):
...
cd frontend-nx ; pnpm exec nx build kppdf-web   ← обязательно последним, exit 0
node start.mjs --nx --no-browser → обход ВСЕХ реестров, скриншоты в
docs/agent-checklists/evidence/TZ-NX-REGISTRY-CRUD-UNIFY/
Baseline ДО claim: nx build kppdf-web exit 0 (иначе STOP — hotfix first).
