...
ЗАВИСИМОСТИ: **TZ-UI-LIGHT-330** — DONE (коммит `35cfc6e3`).

⚠️ **Коррекция от 2026-08-09 05:45.** TZ-330 был реализован из версии промпта **до**
правки про `--color-on-gold`, поэтому в `main` сейчас лежит
`default: 'bg-gold text-ink …'` (`button.component.ts` L14). В тёмной теме
`--color-ink` ≈ `oklch(0.95)`, золото ≈ `oklch(0.84)` → контраст **≈1:1**, главная
кнопка нечитаема. Это **живая регрессия на `main`**, и она поднимается в P0 этого TZ:
токен `--color-on-gold` вводится **здесь** (ШАГ 0), а не в 330.
`--color-paper-raised` и `--color-rule-strong` из 330 в файле уже есть — их не трогать.
...
ШАГ 0 — P0: ввести токен и снять регрессию главной кнопки

В `@theme inline` добавить **тема-инвариантный** токен (намеренно без `-override`:
золото светлое в обеих темах, значит лейбл всегда тёмный):

```
--color-on-gold: oklch(0.20 0.012 260);
```

`button.component.ts` L14: `bg-gold text-ink …` → `bg-gold text-on-gold …`.
Контраст становится ≈6:1 в light и ≈9:1 в dark вместо текущих ≈1:1 в dark.

ШАГ 1 — P0: единый лейбл на золотой заливке (обе темы)

Свип по остальным местам золотой заливки.
...
0. Токен `--color-on-gold` объявлен; `button.component.ts` использует `text-on-gold`;
   главная кнопка читаема в **тёмной** теме (сейчас на `main` она там невидима).
