...
После пустой очереди: короткий отчёт «готово предложить деплой / desktop publish» и **стоп**. Деплой и ZIP **не** запускать.
...
3) Короткий отчёт PO:
   ...
   - HEAD commit
   - «готово предложить warm deploy сайта и отдельно desktop ZIP publish»
