# TZ-AUDIT-QA-ADMIN: Admin devices + roles CRUD
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-DEALS
**SIZE:** S
**PACK:** WAVE-QA-COVERAGE-2026-09-17
**LAYER:** docs/audit
**PAGES:** /admin ; /admin/devices ; /admin/roles
**PAGE_DOCS:** admin-devices.page.md ; admin-roles.page.md
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/admin ; device-enrollment ; role ; permissions ; user ; role-org ; role-counterparty`.
...
- Routes in scope: /admin redirect, /admin/devices, /admin/roles
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-ADMIN.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-ADMIN.done.md` + Executor report (≤15 lines). Commit docs only.
