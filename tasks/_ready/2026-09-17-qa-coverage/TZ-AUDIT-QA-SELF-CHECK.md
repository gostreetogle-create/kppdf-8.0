# TZ-AUDIT-QA-SELF-CHECK: Freeze checklists + 0 missing
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-ORPHAN-BE
**SIZE:** S
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/*`.
...
- Routes in scope: all inventory NX leaves + 93 BE
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-SELF-CHECK.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
5a. Summary Phase 1–3 DONE; top 10 TZ candidates; remove all pending placeholders; WAVE CLOSED note in WAVE-MAP.
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-SELF-CHECK.done.md` + Executor report (≤15 lines). Commit docs only.
