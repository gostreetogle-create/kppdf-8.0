# TZ-AUDIT-QA-REGISTRIES: Registries platform + dictionary redirects
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-STUDIO
**SIZE:** S
**PACK:** WAVE-QA-COVERAGE-2026-09-17
**LAYER:** docs/audit
**PAGES:** /registries ; /registries/:registryKey ; /dictionaries/text-block-categories
**PAGE_DOCS:** registries.page.md ; dictionaries.page.md
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/registry ; dictionary-label ; (keys behind platform)`.
...
- Routes in scope: /registries, /registries/:registryKey, redirect /dictionaries/text-block-categories
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-REGISTRIES.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
5a. Не разворачивай все keys — докажи platform + 2–3 живых key chain; остальные registry keys → №2 Manual/platform или Verified platform pattern.
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-REGISTRIES.done.md` + Executor report (≤15 lines). Commit docs only.
