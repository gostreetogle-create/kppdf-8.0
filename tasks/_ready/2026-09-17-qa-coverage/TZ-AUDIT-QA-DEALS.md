# TZ-AUDIT-QA-DEALS: Counterparties / contracts / proposals
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-REGISTRIES
**SIZE:** S
**PACK:** WAVE-QA-COVERAGE-2026-09-17
**LAYER:** docs/audit
**PAGES:** /counterparties ; /contracts ; /contracts/:id ; /proposals ; /proposals/list ; /proposals/create
**PAGE_DOCS:** counterparties.page.md ; contracts.page.md ; kp-workspace.page.md
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/counterparty ; contract ; quotation`.
...
- Routes in scope: /counterparties, /contracts*, /proposals* (create→studio redirect)
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-DEALS.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
5a. Explicit gap notes: proposal vs Quotation naming; create redirect to studio.
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-DEALS.done.md` + Executor report (≤15 lines). Commit docs only.
