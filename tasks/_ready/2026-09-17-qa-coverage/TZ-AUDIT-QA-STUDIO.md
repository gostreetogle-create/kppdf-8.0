# TZ-AUDIT-QA-STUDIO: Studio list/templates/editor + doc pipeline
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-WAREHOUSE
**SIZE:** S
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/studio-document ; document-template ; document-template-category ; document-render ; text-block ; text-block-category ; table-template ; template-block ; generated-document`.
...
- Routes in scope: /studio, /studio/templates, /studio/:id (+ text/table block registries if linked)
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-STUDIO.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-STUDIO.done.md` + Executor report (≤15 lines). Commit docs only.
