# TZ-AUDIT-QA-PRODUCTION: Production cockpit / Gantt read path
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-SHIPPING
**SIZE:** S
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/work-type ; worker ; work-order ; work-order-operation ; work-center ; routing-step`.
...
- Routes in scope: /production (+ ProductionReadFacade / Gantt consumers)
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-PRODUCTION.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
5a. Только read-path фактически используемый NX; unused → defer ORPHAN.
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-PRODUCTION.done.md` + Executor report (≤15 lines). Commit docs only.
