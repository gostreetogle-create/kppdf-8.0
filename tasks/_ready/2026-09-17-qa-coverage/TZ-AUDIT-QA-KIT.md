# TZ-AUDIT-QA-KIT: Kit routes classify + authGuard smell
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/(none required)`.
...
## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-KIT.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
5a. Confirm evidence for missing authGuard; leave product decision to TZ-NX-KIT-AUTH-GUARD — здесь только classify №1 design-kit public OR №2 security smell.
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-KIT.done.md` + Executor report (≤15 lines). Commit docs only.
