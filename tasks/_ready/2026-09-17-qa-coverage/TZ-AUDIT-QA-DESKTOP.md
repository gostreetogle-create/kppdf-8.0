# TZ-AUDIT-QA-DESKTOP: Desktop pairing + BE desktop + OS №2
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-ADMIN
**SIZE:** S
**PACK:** WAVE-QA-COVERAGE-2026-09-17
**LAYER:** docs/audit
**PAGES:** (shell PairingDialog) ; desktop app
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/desktop ; device-enrollment (cross-ref)`.
...
- Routes in scope: NX PairingDialog + backend desktop + desktop/ areas from inventory §3
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-DESKTOP.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
5a. MCP/import/AI runner → №2 Manual/OS unless fully code-traced.
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-DESKTOP.done.md` + Executor report (≤15 lines). Commit docs only.
