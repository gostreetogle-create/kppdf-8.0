# TZ-AUDIT-QA-WAREHOUSE: Warehouses / storage / stock movements
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-SUPPLY
**SIZE:** S
**PACK:** WAVE-QA-COVERAGE-2026-09-17
**LAYER:** docs/audit
**PAGES:** /warehouses ; /storage-items ; /stock-movements
**PAGE_DOCS:** warehouses.page.md ; storage-items.page.md ; stock-movements.page.md
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/warehouse ; storage-item ; stock-movement ; inventory ; reservation`.
...
- Routes in scope: /warehouses, /storage-items, /stock-movements
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-WAREHOUSE.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-WAREHOUSE.done.md` + Executor report (≤15 lines). Commit docs only.
