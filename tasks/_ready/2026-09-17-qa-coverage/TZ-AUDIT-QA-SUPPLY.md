# TZ-AUDIT-QA-SUPPLY: Supply + supply-requests
...
**ЗАВИСИМОСТИ:** TZ-AUDIT-QA-PRODUCTION
**SIZE:** S
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/supply ; purchase-request ; purchase-order`.
...
- Routes in scope: /supply, /supply-requests
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-SUPPLY.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-SUPPLY.done.md` + Executor report (≤15 lines). Commit docs only.
