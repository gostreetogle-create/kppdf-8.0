# TZ-AUDIT-QA-ORDERS: Orders list/create/detail + hub tray
...
**ЗАВИСИМОСТИ:** Auth/Shell/Home slice DONE (checklists)
**SIZE:** S
**PACK:** WAVE-QA-COVERAGE-2026-09-17
**LAYER:** docs/audit
**PAGES:** /orders ; /orders/create ; /orders/:id
**PAGE_DOCS:** orders.page.md ; home.page.md
...
## Domain preflight
Проверено (открой при claim): inventory §1–2; релевантные `page.md`; NX pages + Pi-* services + `backend/src/modules/order ; order-closing ; order-task ; production-order (read refs)`.
...
- Routes in scope: /orders, /orders/create, /orders/:id (+ hub tray ship/cancel/kit from Home)
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-ORDERS.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
5a. Переклассифицируй Home hub tray ship/cancel/kit из №2 placeholder в №1/№2 по факту.
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-ORDERS.done.md` + Executor report (≤15 lines). Commit docs only.
