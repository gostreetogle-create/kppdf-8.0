# TZ-AUDIT-QA-ORPHAN-BE: Sweep all 93 BE modules
...
- Routes in scope: every backend/src/modules/* from inventory
- Placeholders в №2 «pending domain audit» — снять/переклассифицировать после этого среза.

## ЧТО ДЕЛАТЬ
1. Claim: `tasks/_active/TZ-AUDIT-QA-ORPHAN-BE.md` + checklist Claim slot `agent_id: freebuff`, `claimed_at` ISO. Чужой claim на те же keys → STOP.
...
5a. Для каждого модуля: NX consumer → №1 ref; else №2 `orphan / no NX consumer` (или Desktop-only). Count must = 93.
6. Archive `tasks/_archive/2026-09/TZ-AUDIT-QA-ORPHAN-BE.done.md` + Executor report (≤15 lines). Commit docs only.
