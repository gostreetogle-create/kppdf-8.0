...
| **TZ-263** | Verifier — добавить `ng build` в run-project-checks (tsc не ловит template) | `.agents/skills/run-project-checks/SKILL.md; .husky/pre-commit; docs/AI-AGENT-GUIDE.md` | — |
| **TZ-264** | Admin-диалоги — unit-тесты для 3 dialog-компонентов | `frontend/src/app/pages/admin/*-dialog.component.spec.ts (NEW)` | TZ-261, TZ-265 |
| **TZ-265** | Admin-страницы — Paper & Ink токен-комплаенс (`text-red-600` → `text-destructive`, hex → токены) | `frontend/src/app/pages/admin/{users-admin,roles-admin}.page.ts; *-dialog.component.ts` | TZ-261, TZ-262 |
