...
| **TZ-248** | Production secrets и credential hygiene | backend/config + deployment docs | — |
| **TZ-249** | Auth entry points и anti-automation baseline | backend/auth + throttling | TZ-248 |
| **TZ-247** | Backend idempotency middleware | backend/common/idempotency + Mongo registration | TZ-249 |
| **TZ-250** | Безопасные file uploads и static assets | backend/uploads + proxy limits | TZ-248, TZ-249 |
| **TZ-251** | Object-level authorization и IDOR audit | backend/controllers + services + tests | TZ-249, TZ-254 |
| **TZ-252** | Auth token storage и refresh contract | frontend/core/auth + backend/auth | TZ-248, TZ-249 |
| **TZ-253** | Dependency scan и deployment security checks | CI + deployment docs | TZ-248 |
| **TZ-254** | RBAC contract и политика доступа | backend authorization + data-model docs | TZ-248, TZ-249 |
| **TZ-255** | Server-side permissions enforcement | backend guards/controllers/tests | TZ-254, TZ-251 |
| **TZ-256** | Capability-aware routes и навигация | frontend auth/routes/layout | TZ-254, TZ-255, TZ-252 |
| **TZ-258** | Protected-page onboarding и RBAC regression contract | docs + audit tests | TZ-254, TZ-255, TZ-256 |
| **TZ-257** | Admin module: users, roles и permissions | backend admin + frontend pages | TZ-254..256, TZ-251, TZ-252, TZ-258 |
| **TZ-259** | Builder geometry contract и свободное перемещение блоков | ✅ DONE 2026-07-31 — archive: `tasks/_archive/2026-07/TZ-259.md.done` | completed builder decomposition verification |
| **TZ-260** | Marquee selection, grouping и group drag | builder canvas + bulk persistence + tests | TZ-259, TZ-264 |
| **TZ-261** | Видимая сетка, magnetic snap и alignment guides | builder canvas + snap engine + tests | TZ-259; TZ-260 optional |
| **TZ-262** | Layers, visibility, lock и element opacity | builder inspector + block model + tests | TZ-259; TZ-260 optional; TZ-261 optional |
| **TZ-263** | Builder interaction QA, keyboard contract и rollout checklist | builder tests + docs + smoke | TZ-259..262, TZ-264 |
| **TZ-264** | Builder undo/redo и безопасный keyboard contract | builder state + autosave + tests | TZ-259 |
