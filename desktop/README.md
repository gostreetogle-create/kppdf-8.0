...
Главный экран — три вкладки: **Подключение | Импорт | AI**.

- **Подключение** — только паринг к сайту: вставка JSON, статус «кто я», связать/отвязать.
- **Импорт** (по умолчанию) — формы Excel, drop/файл, сопоставление колонок, inbox агента, отправка/отчёт.
- **AI** (TZD-75: 0.5.8) — сверху **обязательный** блок «Подключение агентов (MCP)»
  (Cursor/LM Studio: порт, mcp.json — это уже работает). Ниже — **«Локальный чат — скоро»**:
  офлайн-модель (~2 ГБ) на 0.5.7/0.5.8 ненадёжна (сборка/пути раннера, TZD-56+), кнопки
  скачивания/запуска убраны, чтобы не спамить ошибками — честное «скоро» вместо этого
  (см. `docs/peer/gemini-desktop-ai-runner-plan.md`, TZD-76 вернёт рабочий вариант по acceptance).
  Облачная модель по API (TokenRouter и др.) — работает уже сейчас, свёрнута в disclosure
  «Подключить облачную модель (по API)». Импорт и Excel-формы работают без модели и без MCP.
  **TZD-77 (0.5.9):** карточка «Чат» растянута на всю высоту колонки; строка Inbox
  (путь/N файлов/«Открыть папку») видна прямо на вкладке AI; чат получает read-only снимок
  имён файлов Inbox в контекст (не содержимое, не запись в БД) — подробности `AI-PROVIDERS.md`.
  **TZD-78 (0.5.10):** из чата можно попросить «разбери файл X» (или нажать кнопку с именем
  файла) — локальная команда без обращения к модели показывает сводку сопоставления и кнопку
  «Открыть в Импорте» (та же студия, тот же HITL — запись в БД по-прежнему только после
  «Отправить» там) — подробности `AI-PROVIDERS.md`.

На вкладке **«Импорт»**: перетащите `.xlsx`, `.xls` или `.csv`
в зону или нажмите «Выбрать файл»; разобранные строки появятся в
таблице предпросмотра. Панель Inbox остаётся вторичной и сохраняет существующий
поток «Разобрать → Создать задачу для ИИ / Предложить строки → Подтвердить».

MCP host (start/stop, порт/LAN, копирование `mcp.json`) — во вкладке **«AI»**, первый блок
«Подключение агентов (MCP)». Переключение вкладок не перезапускает host и не стирает pairing.
...
Локальный помощник (вкладка AI, было «AI-раннер») с 0.5.6 — resource `ai-runner.mjs` (TZD-56), не MCP.
