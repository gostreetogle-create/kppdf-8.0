...
- **AI** (TZD-74: честный порядок) — сверху **обязательный** блок «Подключение агентов (MCP)»
  (Cursor/LM Studio: порт, mcp.json — это уже работает); ниже **опциональный** локальный
  помощник (офлайн-модель ~2 ГБ: скачать/запустить/открыть папку моделей) для локального чата и
  подсказок по нестандартным заголовкам импорта; облачная модель по API — свёрнута в disclosure.
