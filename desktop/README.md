...
- **AI** — локальная модель (скачать/запустить/открыть папку моделей) + MCP для агентов (Cursor/LM Studio: порт, mcp.json). Импорт и Excel-формы работают без модели и без MCP.
...
MCP host (start/stop, порт/LAN, копирование `mcp.json`) — во вкладке **«AI»**, блок
«MCP для агентов». Переключение вкладок не перезапускает host и не стирает pairing.
