...
## Чат предлагает mapping → Импорт HITL (TZD-78, 0.5.10)

Из чата теперь можно попросить разобрать файл Inbox без переключения на
вкладку «Импорт»:

1. **Кнопка или команда.** Над историей чата — ряд кнопок с именами файлов
   Inbox («Разобрать файл: …»); либо текстом: «Разбери файл X.xlsx»,
   «импортируй поставщики.csv» и т.п. — распознаётся локальной эвристикой
   (`core/ai/chat-inbox-intent.ts`, слово-триггер + имя/основа имени файла),
   **без обращения к модели** — работает даже без настроенного AI-провайдера.
2. **Read-only сводка.** Файл читается через существующий `auditInboxFile`
   (тот же, что кнопка «Разобрать» на вкладке «Импорт»); лучшая таблица
   подбирается локальным классификатором `analyzeTables`
   (`core/ai/suggest-mapping.ts` → `pickBestTableSuggestion` +
   `buildInboxMappingSummary`) — та же логика без AI, что и fallback-ветка
   «Предложить сопоставление» на вкладке «Импорт». Ничего не пишется в API.
3. **CTA «Открыть в Импорте».** Появляется под сводкой в чате; переключает
   вкладку на «Импорт» и вызывает тот же `auditFile()`, что и ручной клик по
   файлу в списке Inbox — студия открывается с уже готовым черновиком
   сопоставления. Запись в БД — только после «Отправить» там (HITL, без
   изменений в write-пути).
