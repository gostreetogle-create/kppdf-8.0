...
> сам и модель грузится в память без «Перезапустить». Модель подсказывает
> сопоставление колонок на вкладке «Импорт» и отвечает в чате — LIMITED_HELPER,
> без записи в базу из чата (TZD-64). Ниже на этой же вкладке — отдельный блок
> «MCP для агентов»: для внешних клиентов (Cursor, LM Studio), не для этого
> чата. Порядок всех шагов подключения — раздел «С чего начать» в
