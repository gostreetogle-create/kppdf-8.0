...
**AI** (TZD-74: MCP — обязательный и первый блок; TZD-75/0.5.8: локальный чат скрыт за
честное «скоро» — ненадёжен в NSIS, см. `AI-PROVIDERS.md`). MCP-блок живёт во вкладке
**AI → «Подключение агентов (MCP)»**; паринг — во вкладке «Подключение». Импорт и
...
(Каталог / Контрагенты / Справочники / Снабжение) → таблица
(Материалы / … / Быстрый заказ / Задачи снабжения)
...
**Скачать с данными (TZD-68):** рядом с «Скачать Excel-форму» — **«Скачать
с данными»**, тот же файл (`kppdf-{table}-export.xlsx`), но лист «Данные»
уже заполнен текущими строками SoT. Пилот V1: только **Материалы** и
**Виды работ**; для остальных таблиц кнопка неактивна. Требует подключения
(аккаунт) — без паринга кнопка неактивна.
...
2. **MCP запускается автоматически** — при подключённом аккаунте десктоп сам
   ...
   не нужен. В блоке «Подключение агентов (MCP)» (вкладка «AI», первый) видны статус
...
1. В Desktop: блок «Подключение агентов (MCP)» (вкладка «AI») → **«Перезапустить»**
...
> packaging/sidecar MCP остаётся follow-up. Локальный помощник (было «AI-раннер») с 0.5.6 —
