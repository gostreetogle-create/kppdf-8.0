...
**AI** (TZD-74: MCP — обязательный и первый блок; локальный помощник — опциональный,
ниже). MCP-блок живёт во вкладке **AI → «Подключение агентов (MCP)»**; паринг — во
вкладке «Подключение». Импорт и Excel-формы работают без модели и без MCP.
