...
## С чего начать (порядок подключения)

Единая последовательность для нового клиента — по шагам, каждый пункт раскрыт в своём документе:

1. **На сайте** (веб-фронтенд): «Подключить десктоп» → «Выпустить ключ» → «Скопировать» —
   получаем паринг-пакет. Детали и формат пакета: [`PAIRING.md`](./PAIRING.md).
2. **В Desktop**: вкладка **«Подключение»** → вставить скопированный пакет → «Подключиться».
   Это единственный обязательный шаг после установки: [`PAIRING.md`](./PAIRING.md).
3. **Опционально — внешний AI-клиент** (Cursor, LM Studio): вкладка **«AI»** → блок
   **«MCP для агентов»** → «Запустить» → «Скопировать mcp.json».
   Список tools и разбор проблем: [`MCP.md`](./MCP.md).
4. **Опционально — локальный чат и помощь при импорте**: вкладка **«AI»** →
   «Открыть чат» (если файл модели уже на диске — открывается сразу) либо
   карточка «Локальная модель» → «Скачать модель». Та же модель подсказывает
   сопоставление колонок на вкладке «Импорт». Модели и требования к ПК:
   [`AI-PROVIDERS.md`](./AI-PROVIDERS.md).

Шаги 3 и 4 независимы друг от друга и не требуются для обычного импорта Excel.
...
После запуска Desktop открывается вкладка **«Импорт»**. Она содержит большую зону
перетаскивания и основную таблицу предпросмотра для `.xlsx`, `.xls` и `.csv`; Inbox
остаётся вторичной панелью ниже. Вкладка **«Подключение»** (в прежних версиях доки —
«MCP»; фактическая подпись кнопки в UI — «Подключение», паринг + вход в MCP-контур)
содержит pairing. Сам MCP host — статус, Start/Stop, URL, порт/LAN и кнопки копирования
`mcp.json` — живёт в блоке **«MCP для агентов»** на вкладке **«AI»** (см. `MCP.md`).
...
`git pull` **перезапустите MCP** (вкладка «AI» → блок «MCP для агентов» → «Перезапустить»), иначе
...
## Локальная модель и чат (вкладка AI)
...
или положить `.gguf` вручную (**любое** имя файла, не только из каталога — см. TZD-63
ниже). Импорт и Excel-формы работают без модели и без MCP.

**TZD-62 (чат):** кнопка **«Открыть чат»** — если файл выбранной модели уже в
`app-data/models`, раннер стартует (или перезапускается) сам с этим файлом,
модель грузится в память, и внизу можно сразу писать сообщения. Если файла
нет — RU-подсказка + те же «Открыть папку моделей» / «Скачать модель», без
падения. Чат — LIMITED_HELPER: не пишет в базу, не деплоит, не берёт задачи;
системный промпт — `desktop/ai/system-prompts/desktop-chat.md`.

**TZD-63 (любой .gguf + скачать без ритуала):** кнопка **«Обновить список»**
сканирует `app-data/models` на любые `.gguf` (не только 3 модели из каталога) —
удобно после копирования файла с флешки под своим именем. Отсев мусора/ярлыков:
размер 200 МБ…20 ГБ и magic-байты `GGUF` в начале файла (`core/gguf-scan.ts`);
что не подошло — видно в списке «Пропущено» с RU-причиной. Найденный файл можно
выбрать в «Файл на диске» — он побеждает выбор из каталога для «Открыть чат» и
«Запустить». «Скачать модель» больше не требует сначала жать «Запустить»: если
раннер не работает, кнопка сама его поднимает без модели, качает файл и сразу
перезапускает уже с ним — обязательного порядка «Запустить → Скачать →
Перезапустить» больше нет.
