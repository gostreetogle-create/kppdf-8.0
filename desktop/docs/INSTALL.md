...
4. **Опционально — помощь при импорте**: вкладка **«AI»** → карточка «Локальная модель» →
   «Скачать модель». Модель не чат: она подсказывает сопоставление колонок на вкладке
   «Импорт». Модели и требования к ПК: [`AI-PROVIDERS.md`](./AI-PROVIDERS.md).
