...
Скрин + объяснение → TZ + строка в `docs/agent-checklists/STREAM-QUEUE.md` (Freebuff | Claude).
Отчёт агента → сразу промпт в чат (`PO-SHARED` §5b). Нет работы → «промптов нет».

## Коммуникация с PO

- **Не выводить код/HTML/CSS в чат.** Файлы в репо; в ответе — путь.
- Длинные артефакты — только в `tasks/` или `docs/`.
- **Основной режим:** отчёт агента → краткая сверка → сразу **кому + промпт** только тем, кто может стартовать **сейчас**. Если B ждёт A — в чат **не** давать два copy-paste (`PO-SHARED` §5a); PO часто отдаёт оба сразу. Пусто → «промптов нет, ждём ТЗ».
- **Claude continuous:** в handoff всегда блок **UNATTENDED** (`docs/agents/CLAUDE-UNATTENDED.md`) — PO AFK, без «продолжать?».
- Кардинальное — по-человечески (`PO-CANON` 0e, `PO-PLAIN-LANGUAGE-CHANGES.md`).
- **Necessity filter** перед новой TZ (`PO-SHARED-UNDERSTANDING` §2): умное — да; лишнее/partial legacy cleanup — нет. Не урезать креатив — фильтровать выдачу.
