...
| `photoIds` + `mainPhotoId` | forms NX (WAVE-NX-CATALOG-PHOTOS P1); upload `POST /photos/upload` | витрина/Гант/списки (cover = `mainPhotoId` ?? `photoIds[0]`) | Product получает `mainPhotoId` в P2; деталь = Material; frame = `Photo.frame` (P3), не второй cropper |
