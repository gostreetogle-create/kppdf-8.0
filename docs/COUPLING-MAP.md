...
| `photoIds` + `mainPhotoId` | forms NX (WAVE-NX-CATALOG-PHOTOS P1); upload `POST /photos/upload` | витрина/Гант/списки (cover = `mainPhotoId` ?? `photoIds[0]`) | Product `mainPhotoId` в BE **DONE P2** (`TZ-NX-PHOTO-P2-PRODUCT-MAIN`: schema/DTO/service validate main ∈ photoIds → 400 RU; duplicate наследует); деталь = Material; frame = `Photo.frame` (P3), не второй cropper |
