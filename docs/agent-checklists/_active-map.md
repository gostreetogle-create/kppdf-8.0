...
- Gates: FE tsc PASS · auth.service 19/19 · auth.interceptor 12/12 · enroll 6/6 · devices-admin 6/6 · login.page 4/4 · permission-labels PASS · eslint/diff-check PASS.
- Archive: `tasks/_archive/2026-08/TZ-AUTH-304.done.md`; lock `tasks/_backlog/TZ-AUTH-304-device-enrollment-ui.lock`; checklist DONE; `_active/TZ-AUTH-304.md` removed.
- Pre-existing (не регрессия): FormProfilesService › isLocked падает на main (TZ-DICT-315).
- IN PROGRESS: none · NEXT: TZ-AUTH-305 (nginx auth_request + rollback; C1: owner break-glass достижимость до включения auth_request).
- HEAD: pushed: yes · Blockers: none · Deploy: НЕ
- `_active/`: empty
...
- Runbook: `docs/ops/RUNBOOK-CLEAN-SYNLOGY-KP3-LOAD.md`
- **Стоп по DANGEROUS-OPS:** нужны VPN off + бэкап + точная фраза «да, разрешаю wipe после бэкапа»
- Порядок: backup → wipe+seed deploy → smoke ZIP v{semver} → pair Desktop → MIG-302 scoped
- NEXT: ждать фразы PO; не жать wipe / deploy.ps1 -Wipe до разрешения
- Ban: wipe без бэкапа; MIG-302 на prod до чистого деплоя; photo load в этом прогоне
...
## Checkpoint 2026-08-12 · Desktop WIP + TZD-40 + TZD-45 DONE (фон свободен)
- DONE: Desktop WIP `d0c6d179` · TZD-40 version gate `f06cecf1` · TZD-45 production/supply read `a158a146`
- IN PROGRESS: none
- NOT DONE: none (очередь фона: Desktop WIP → TZD-40 → TZD-45 — всё закрыто)
- NEXT: idle — фоновый агент свободен; не выдумывать z-series / production UI / склад 304
- HEAD: `a158a146` pushed: yes
- Blockers: none
- Deploy: NO (баннер версий TZD-40 появится только после warm deploy env)
- _active/: пусто · не трогал `proposal-create*` / `proposals.page*` / `quotation*` / `document-template*` / `ruvector.db`
...
- Commit/push: `fbd6ba04` on `origin/main`; deploy НЕ. NEXT: TZD-44; TZD-45 parked.
