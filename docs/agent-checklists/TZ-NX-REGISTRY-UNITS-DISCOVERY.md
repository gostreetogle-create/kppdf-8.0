# TZ-NX-REGISTRY-UNITS-DISCOVERY checklist

> Status: **DONE**
> Marker: archived as `tasks/_archive/2026-08/TZ-NX-REGISTRY-UNITS-DISCOVERY.done.md`
> Mode: **analysis-only** — no product code/config changes
...
- [x] Проверены `tasks/_active/` (пусто до этого claim) — конфликтов нет
- [x] TZ прочитан: `tasks/TZ-NX-REGISTRY-UNITS-DISCOVERY.md` + execution prompt
- [x] Claim slot заполнен; Status = CLAIMED / IN PROGRESS
- [x] `tasks/_active/TZ-NX-REGISTRY-UNITS-DISCOVERY.md` на месте

## Acceptance (discovery deliverables)

- [x] Legacy inventory (entity/model/schema, DTO, endpoints+methods, поля+validation, filters/sorting/pagination, CRUD+archive/delete, permissions, relations/FK)
- [x] NX mapping (data-access location, API contract/DTO types, data-source/adapter boundary, RegistryDefinition mapping, route/page structure, required UI primitives, loading/error/empty/retry)
- [x] Risks (semantic mismatches, soft-delete/archive rules, authorization gaps, migration hazards, invented fields/permissions guardrails)
- [x] Ordered implementation plan (exact next tasks, parallel vs sequential, first vertical slice)
- [x] Никаких правок `frontend/**`, `backend/**`, `frontend-nx/**`; никаких новых routes/permissions/DTO/DB fields/deps; credentials не раскрыты — подтверждено `git status --porcelain -- frontend backend frontend-nx` до/после: единственные изменения в этих деревьях предшествуют этой сессии (см. Executor report)

## Integrity slot (до READY / archive)

- [x] Тип изменения: docs-only (analysis/discovery report)
- [x] FIC §A–E: N/A — нет кода, нет нового module/permission/MCP
- [x] page.md / PAGE-TZ-INDEX: N/A — не создаётся новая страница/route в этой TZ
- [x] SECTION-READINESS: N/A
- [x] Чужой WIP не в коммите; conflict keys: только новые docs/tasks файлы этой TZ (см. список ниже), никаких пересечений с чужими active claims
- [x] Coupling map: N/A (discovery, не меняет общее поле/статус)
- [x] Канон: docs/DOCS-INTEGRITY.md — отчёт хранится в checklist + archive, без изменения кода

## Gates (факт)

- Не применимо — analysis-only, изменений кода/конфигурации нет, поэтому typecheck/test/lint/architecture:check не запускались (TZ явно запрещает менять код).

## Executor report

Прочитано (read-only): `tasks/_archive/2026-08/TZ-NX-REGISTRIES-PLATFORM.done.md`,
`TZ-NX-REGISTRIES-NAV-AND-DEMO-REVIEW.done.md`, `TZ-NX-SHELL-CANON.done.md`,
`docs/pages/registries.page.md`, `docs/pages/measurements-group.page.md`,
`docs/pages/units.page.md`; весь `frontend-nx/apps/kppdf-web/src/app/pages/registries/**`
(model/registry.types.ts, model/registry-query-state не открывался целиком —
не требовалось для discovery, registry-detail.page.ts, data/units.registry.ts,
data/departments.registry.ts, data/fixture-registry-data-source.ts,
data/registries.catalog.ts); `frontend-nx/libs/data-access/**` (структура +
`admin/pi-roles.service.ts`, `capabilities/page-acl.ts`, admin route guard в
`app.routes.ts`), `frontend-nx/libs/util/http/src/lib/silent-http.ts`;
backend `modules/unit/**` целиком (schema/service/controller/module/оба DTO/
controller.spec), `common/seed/units.seed.ts`, `app.module.ts` (registration),
`common/decorators/roles.decorator.ts`, `common/interceptors/org-scope.interceptor.ts`,
`database/soft-delete.plugin.ts`, `database/soft-delete-coverage.spec.ts`,
`database/database.module.ts`, `modules/storage-item/storage-item.service.ts`
(precedent для soft-delete no-op бага), `main.ts` (ValidationPipe), grep по
`rbac-contract.ts`/`permission-types.ts` (нет `unit`); legacy frontend
`pages/dictionaries/units.service.ts` и `measurements-group.page.ts` целиком.

Изменений в `frontend/**`, `backend/**`, `frontend-nx/**` в этой сессии не
делал — только Read/Grep/Glob. `git status --porcelain -- frontend backend
frontend-nx` после сессии показывает исключительно файлы, уже изменённые ДО
старта этой TZ (видно в исходном gitStatus снапшоте: `rbac-contract.ts`,
`permissions.guard.ts`, `auth.module.ts`, `auth.service.ts`,
`jwt.strategy.spec.ts`, `jwt.strategy.ts`, `build-info.ts`,
`studio-panel-layers.component.ts`, `studio-ribbon.component.ts`) — не мои
правки. `frontend-nx/` показывает `??` (untracked) в `git status`, что не
связано с чтением файлов в этой сессии.

Полный discovery-отчёт — см. Auditor report ниже.
...
1. **DELETE row action would silently do nothing** (see legacy defect above). Porting it as a `RegistryRowAction { destructive: true, confirm: {...} }` calling `PiUnitsService.remove()` would faithfully reproduce a real backend bug — the row would vanish from the current page (client re-render after `reload()`... actually **no**: since the document is untouched, `reload()` would re-fetch it and it would **reappear**, which is a worse, more visible bug than in the legacy page's local-array + `httpResource.reload()`). **Must be surfaced to PO before wiring**: either fix `UnitService.remove()` first (small, isolated backend TZ, precedent = `storage-item.service.ts` hard-delete fix or add the missing `deletedAt` prop + rely on the plugin), or deliberately omit the delete action from the first NX slice and only wire the (working) `isActive` toggle.
...
- [x] Review diff перед закрытием — самопроверка: только docs/tasks файлы изменены (`tasks/_active/TZ-NX-REGISTRY-UNITS-DISCOVERY.md`, этот checklist, archive `.done.md`), `git status` подтверждает отсутствие новых правок в frontend/**, backend/**, frontend-nx/** (single-agent TZ, без wave review inbox)

## Closeout (после PASS)

- [x] archive `tasks/_archive/2026-08/TZ-NX-REGISTRY-UNITS-DISCOVERY.done.md`
- [x] удалить `tasks/_active/TZ-NX-REGISTRY-UNITS-DISCOVERY.md`
- Status = DONE
