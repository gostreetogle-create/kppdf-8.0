...
- **Pre-commit**: `.husky/pre-commit` → `scripts/smoke/supply-gate.mjs` — при
  ...
  (полный прогон вручную, командой ниже).
- Локально: `pnpm smoke:supply` (полный smoke), `pnpm gate:supply` (гейт,
  `--force` — независимо от diff).
- GitHub Actions / облачный CI **не используется** — политика проекта
  (`deploy/synology/README.md`): GitHub только хранит репозиторий, без прогонов
  и биллинга. Воркфлоу `supply-smoke.yml` удалён 2026-08-21.
