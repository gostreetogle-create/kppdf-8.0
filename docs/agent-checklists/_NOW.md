...
**TZ-UX-FORM-308** — DONE by `claude` (2026-08-22), SHA `0047dad9`: `frontend/src/app/shared/ui/quick-create/field-capacity.ts` — один `FIELD_CAPACITY` реестр теперь явно для QuickCreate (kind B) **и** FullEditor (kind C), не вторая таблица; добавлены `subcategory: 'sm'`/`ralCode: 'sm'` (FullEditor-only); `index.ts` экспортирует `colSpanClass`/`controlMaxClass`/`CAPACITY_CONTROL_MAX_CLASS`; новый `field-capacity.spec.ts` (6/6 PASS). Попутно исправлена регрессия в `quick-create-dialog.component.spec.ts` (строгий QC-only allowlist assertion конфликтовал с целью TZ — добавлен явный `fullEditorOnlyKeys`). `product-form-dialog`/`module-form-dialog` (309/310) не тронуты. Архив `tasks/_archive/2026-08/TZ-UX-FORM-308.done.md`; gates PASS (FE tsc, field-capacity 6/6, lint 0 err; regression quick-create-dialog 14/14); deploy НЕ.
**tasks/ hygiene** — DONE by `claude` (2026-08-22): по просьбе PO корень `tasks/` вычищен до канона (`README.md`, `PROMPT-RESUME-ANY.md`, `PROMPT-UNIVERSAL-CONTINUOUS.md`, `PROMPT-FREEBUFF-TASKS-DRAIN.md`) — все spent PROMPT-файлы (DEPLOY-303, DESKTOP-AI-API/CHAT, FREEBUFF wave-A/B, CORE-303-SCHEMA, SUPPLY-304-design) перенесены в `tasks/_archive/2026-08/prompts-spent/`; `tasks/_active/` очищен от архивированных маркеров (OPS-318/319, PARTY-305, CORE-303 — все DONE); фантомные черновики `_backlog/TZ-ORDERS-307`/`TZ-PARTY-305`/`TZ-SUPPLY-305` удалены (те же TZ ID давно реализованы и заархивированы под другими путями). Заодно закоммичена ранее незакоммиченная, но уже проверенная hygiene-работа (OPS-320-стиль: спент-промпты/дуп-спеки в `_archive/2026-08/{prompts-spent,specs-dup-root}/`, deleted root/`_backlog` копии архивированных TZ) — каждое удаление сверено с реальным `*.done.md` в архиве перед коммитом. deploy НЕ.

**TZ-DEPLOY-303** — DONE by `claude` (2026-08-22), SHA `f2a720f6`: `docs/agent-checklists/PRE-DEPLOY-2026-08-19.md` обновлён под текущий `origin/main` — target SHA `832eeab66f25e1cefa080f8a5a4fa99be896a3c3` (69 коммитов с `ba98a4a5`, §B суммирует темы: Desktop AI 57→65, стол/заказы, снабжение, каталог/UI, backend hygiene, TEST-420, вся micro-type волна UI-408…417 DONE); §C свежие гейты PASS (FE/BE/Desktop tsc — FE перепрогнан на финальном SHA после того, как Freebuff волна B landing дважды во время сборки таблицы; FE focused jest 74/74 desk+orders; BE focused jest 88/88 desk-note+order; desktop tsx --test 11/11 chat-url+snippet-parse); §D installer 0.5.6 найден, но STALE — собран до TZD-62→65, нужен republish перед выдачей ссылки на кати; §E review-рой не прогонялся (только docs-refresh); §F Deploy — **деплой не выполнялся**, ничего не запускалось (`deploy.ps1`/SSH/wipe). Архив `tasks/_archive/2026-08/TZ-DEPLOY-303.done.md`; deploy НЕ.
...
**TZ-UI-414** — DONE by `freebuff-1` (2026-08-22): dashboard lane helper `text-[10px]` raised to `text-[11px]`; archive `tasks/_archive/2026-08/TZ-UI-414.done.md`; SHA `382f904e`; gates FE tsc + scoped search PASS; deploy НЕ.
...
**TZ-DESK-423** — DONE by `freebuff` (2026-08-22): shared desk tray is an operator workspace with `Добавить изделие`, `panel=bom` + `variant=items`, click-gated `draft→confirmed` PATCH only on desk, and compact right-side disclosure controls; archive `tasks/_archive/2026-08/TZ-DESK-423.done.md`; SHA `57b6a7fa`; gates FE tsc, focused Jest 41/41, lint PASS; deploy НЕ.
...
**TZ-UX-345** — DONE by `freebuff` (2026-08-23T00:15:00+03:00): chrome-rail visible at ≥1024px (=lg padding 64px), added subtle menu-strip background (paper-2 + inner border), fallback toolbar thresholds 1680→1024 on products/modules/materials; archive `tasks/_archive/2026-08/TZ-UX-345.done.md`; gates PASS (FE tsc, jest 99/99, lint 0 errors); deploy НЕ.
**TZ-UX-FORM-310** — DONE by `freebuff` (2026-08-23T00:45:00+03:00): module FullEditor field packing — name/article 12-col (8/4 span), dimensions+weight single band with max-w constraints + tabular-nums; archive `tasks/_archive/2026-08/TZ-UX-FORM-310.done.md`; gates PASS (FE tsc, jest 7/7, lint 0); deploy НЕ.
**TZ-UX-FORM-311** — DONE by `freebuff` (2026-08-23T01:10:00+03:00): material FullEditor field packing — name/article 12-col (8/4 span), unit/sku/kind by capacity spans, weight+price max-w + tabular-nums, assortment/standard/grade 12-col sm spans, dim value nano max-w; archive `tasks/_archive/2026-08/TZ-UX-FORM-311.done.md`; gates PASS (FE tsc, jest 47/47, lint 0); deploy НЕ.
**TZ-UX-FORM-313** — DONE by `freebuff` (2026-08-23T01:30:00+03:00): order form flyout narrow fields — number max-w:16rem, date max-w:11rem, priority/status max-w:12rem, phone max-w:14rem in 12-col quick-party, qty/price nano/xs max-w + tabular-nums; archive `tasks/_archive/2026-08/TZ-UX-FORM-313.done.md`; gates PASS (FE tsc, jest 12/12, lint pre-existing in people-form-dialog); deploy НЕ.
**TZ-CORE-303** — DONE by `freebuff` (2026-08-22T23:10:00+03:00): restored @Schema decorator on 17 schema files (20 occurrences), backend boots cleanly, health 200; archive `tasks/_archive/2026-08/TZ-CORE-303.done.md`.
