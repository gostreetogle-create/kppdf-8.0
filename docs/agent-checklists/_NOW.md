...
**TZ-UI-404** — DONE by `claude` (2026-08-22T13:35:00+03:00); commit pending; archive `tasks/_archive/2026-08/TZ-UI-404.done.md`; Клиенты/Каталог/Снабжение/Цех (10 маршрутов) переведены `[chips]`→`[toc]` (чёрная TOC-строка, золотая пустая), по канону contracts/proposal-create; gates PASS (FE tsc, jest 420/424 — 4 pre-existing unrelated failures в production-read.facade.spec.ts, lint, architecture); browser primary PASS (CDP smoke все 10 маршрутов, `reports/TZ-UI-404-toc-parity-smoke.json`); deploy НЕ.
**TZ-UI-406** — DONE by `claude` (2026-08-22T14:45:00+03:00); commit pending; archive `tasks/_archive/2026-08/TZ-UI-406.done.md`; PO-flagged gap: Проект (`/design`+`/design/combine`) — двух разных nav shell унифицированы под `[toc]` (тот же канон TZ-UI-404); gates PASS (tsc, jest 29/29, eslint); browser primary PASS (CDP smoke); deploy НЕ. **Начат большой PO-заказ «до deploy readiness»** — план: см. следующие записи.
**TZ-CRASH-401** — DONE by `claude` (2026-08-22T15:20:00+03:00); commit pending; archive `tasks/_archive/2026-08/TZ-CRASH-401.done.md`; новый `scripts/full-route-crash-sweep.mjs` (CDP, все 50 routes) нашёл реальный краш `/dictionaries/form-profiles` (Lucide string-icon lookup без `.pick()` регистрации — checkbox/badge/card); fix: `app.config.ts` `LucideAngularModule.pick({Check,Minus,ArrowUpRight})`; sweep 1/50→0/50; gates PASS (tsc, jest 60/60, eslint); deploy НЕ. **PO deploy-readiness order: 4 параллельных worktree-агента запущены** (TZ-RBAC-401 roles audit, TZ-UI-407 design/theme polish, TZ-DESKTOP-401 desktop audit, TZ-TEST-401 coverage pass) — все isolated `.worktrees`, NOT merged yet, coordinator (this session) reviews+merges each on completion.
