...
**TZ-CORE-303** — READY (2026-08-22): локальный backend не бутит. Незакоммиченный WIP снял `@` у `@Schema` в ~17 `*.schema.ts` (после CORE-302 `softDelete: false`). Симптом: `StrictModeError Path "slug" is not in schema` на `DocTypeService.onModuleInit`. TZ `tasks/TZ-CORE-303-restore-schema-decorator.md`; промпт `tasks/PROMPT-FREEBUFF-CORE-303-SCHEMA-2026-08-22.md`. Frontend :4200 при этом живой. deploy НЕ.
