...
**TZ-UX-FORM-308** — DONE by `claude` (2026-08-22): `frontend/src/app/shared/ui/quick-create/field-capacity.ts` — один `FIELD_CAPACITY` реестр теперь явно для QuickCreate (kind B) **и** FullEditor (kind C), не вторая таблица; добавлены `subcategory: 'sm'`/`ralCode: 'sm'` (FullEditor-only); `index.ts` экспортирует `colSpanClass`/`controlMaxClass`/`CAPACITY_CONTROL_MAX_CLASS`; новый `field-capacity.spec.ts` (6/6 PASS). Попутно исправлена регрессия в `quick-create-dialog.component.spec.ts` (строгий QC-only allowlist assertion конфликтовал с целью TZ — добавлен явный `fullEditorOnlyKeys`). `product-form-dialog`/`module-form-dialog` (309/310) не тронуты. Архив `tasks/_archive/2026-08/TZ-UX-FORM-308.done.md`; gates PASS (FE tsc, field-capacity 6/6, lint 0 err; regression quick-create-dialog 14/14); deploy НЕ.
