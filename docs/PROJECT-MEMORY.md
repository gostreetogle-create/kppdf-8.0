...
> TZ-OPS-302 · docs-only · канон агента: `GEMINI.md` + `docs/AI-AGENT-GUIDE.md` + `docs/PO-DIARY.md` §1–§4
...
Если задача уже сделана в другом месте — не делай повторно; сверь очередь `_active-map`.

## 2. Ритуал 60 секунд (каждый старт TZ)

1. Рабочая папка: `D:\kppdf-8.0` на ветке `main` — **не** `.freebuff/worktrees/*`.
...
5. Сверь `_active-map.md` + чужие `_active` **CONFLICT KEYS** — пересечение = STOP/DEFERRED.
...
| PO, планка качества, вкус | `docs/PO-DIARY.md` §1–§4 (вкус) |
...
- Не работать в `.freebuff/worktrees/*` и не коммитить оттуда в main.
