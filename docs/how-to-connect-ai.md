...
- ❌ НЕ считай локальный PASS доказательством боевого. Deploy/wipe — только слово PO.
- ❌ НЕ ходи в prod «посмотреть» / «только прочитать» без явной команды PO.
...
- Перед предложением деплоя: чем боевые данные/env отличаются от тех, на которых проверял? Нет ответа → приёмки не было.
