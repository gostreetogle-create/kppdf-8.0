...
| `/studio`, `/studio/templates`, `/studio/:id` | **document-studio.page.md** | **WAVE-DOC-STUDIO** Wave 0 DONE (ADR); **101 READY** extract; 201a…1101 planned; **WAVE-DOCSTUDIO-CHROME-IA C1–C4 DONE (2026-09-06)** — три раздела, крошки, rail-действия; **S47-COLUMN-MAP-PARITY DONE (2026-09-08)** — bind by column.key, rehydrate liveRows on column change, hidden-column filter parity; **S48-TABLE-PHOTO-CELLS DONE (2026-09-08)** — photo column renders `<img>`/«Нет фото» on canvas + PDF; **console hygiene (2026-09-11, `PROMPT-CLAUDE-STUDIO-CONSOLE-HYGIENE`):** Lucide NX pick **DONE**; template-picker delete icon `pi-icon-btn`/`pi-icon-btn-danger` **DONE**; sync-quotation orphan heal + no `window.confirm` archive — pending; **WAVE-DOCSTUDIO-EDITOR-DECOMP READY (2026-09-14)** — Phases 1–4 pack `tasks/_ready/2026-09-14-docstudio-editor-decomp/` (facade in-place → util → UI → facade-to-features; page stays in app; Phase 5 UI-split PARK) · audit `docs/audits/2026-09-14-docstudio-god-component-decomposition.md` |
...
| `/counterparties` | counterparties.page.md | **FORMS-316 READY** vat/days number; **NAV-301** thin list; **PARTY-301 DONE**; **PARTY-303 DONE**; **MIG-304** поле «Почта» (git); **MIG-307 BLOCKED** до кати `da01f1e5` · **TZ-NX-HUB-01 DONE** hub expand (заказы/КП/договоры/объекты) + icon actions (`WAVE-NX-HUB-TABLE-PARITY`) |
| `/design`, `/design/combine`, `/supply`, `/shipping` | **design.page.md**; **design-combine.page.md**; supply.page.md; shipping.page.md | **COMBINE-401…415**; **supply** live; **shipping** реестр (не stub); **TZ-SHIP-433 READY** отмена до dispatch; **SWEEP-401** ship; **TZ-SHIP-440 READY** warehouse select (не ObjectId); **TZ-UX-440 READY** supply «Почта организации»; **TZ-UX-443 LIVE** content inset; **TZ-SUPPLY-443 DONE** org `+` btn canon · **WAVE-NX-SUPPLY**: S0 DONE (kit-reserve BE); **TZ-NX-SUPPLY-S1-PAGE DONE** — `/supply` NX live registry (no mock), `?orderId=`, transitions, explode/create; S2 (hub confirm) READY · **WAVE-NX-SHIPPING**: S0 DONE (data-access); **TZ-NX-SHIP-S1-REGISTRY DONE** — `/shipping` NX live registry (nav «Склад»), create/dispatch/cancel/edit/add-doc dialogs; S2/S3 (hub) READY · **TZ-NX-HUB-03 DONE** dense table + compact status CTA (`WAVE-NX-HUB-TABLE-PARITY`) |
...
| `/contracts` | contracts.page.md | **SALES-310 DONE** — dark Deals TOC; **Contract file wave C1–C5 DONE** (`contractStatus` + attachment API); NX UI PARK |
...
| `/proposals` | **proposals.page.md** | **FORMS-316 READY**; **SALES-313 DONE** family; **NX KP Family S40–S48 DONE** (types/API/hide variants/expand/attach/sync/studio/convert guard/operator docs) · **KP PDF from list DONE** — row action calls `POST /quotations/:id/pdf`; S47 `2cdf013e` |
...
| `/dictionaries/text-block-categories` | **OPS-305 DONE** → `text-block-categories.page.md` | DOC-334; DICT-307; **DICT-310**; **DICT-312**; WAVE-PAGE-DOCS-GAPS; **UX-342 DONE** drop dead pager total; **TZ-NX-TEXT-CAT-NX-CRUD DONE (2026-09-11)** — NX route live (was dead nav), master-detail roots→subcategories CRUD |
...
| `/login` | login.page.md | TZ-AUTH-301 notice; **AUTH-306** owner break-glass; **AUTH-305 DONE**; AUTH-307 park; **TZ-COMP-401** formula + privacy link; **TZ-UI-415 READY** footer 11px; **WAVE-AUTH-SMELLS READY** POSTLOGIN-HOME · PRIVACY-LINK · DEMO-PASSWORD-TITLE (`tasks/_ready/2026-09-17-auth-smells/`) |
...
| **frontend-nx-only** `/registries`, `/registries/:registryKey` | registries.page.md | **TZ-NX-REGISTRIES-MASTER-TABLE-UX DONE** — master table + inline detail panel (no separate detail route); `units` real API (`GET/PATCH /units`), `departments` demo fixture; no legacy equivalent, no domain permissions · **WAVE-NX-MODULE-WT-DAYS READY** — people skills без `Nд` (`TZ-NX-REGISTRIES-WORKER-SKILLS-NO-DAYS`) + module WT days SoT (`TZ-NX-MODULE-WT-DAYS-SOT`) |
...
| WAVE-NX-DOCSTUDIO-LIST-TEMPLATES-CLEAN DONE (2026-09-12) | `/studio`: убрана «Новое КП» (дубль `/proposals` «Создать в студии») — остались Шаблоны \| Из шаблона \| Создать документ; `document-templates` `findAll` теперь всегда исключает soft-deleted + системный sentinel `system-sentinel-blank-a4` (было: admin без org видел sentinel каждой организации); `ensureBlankA4Sentinel` сам дедуплицирует (oldest keep, soft-delete extras) + safe от create-race; unique partial index; обе «нет шаблонов» копии — честные («Нет сохранённых шаблонов…») | WAVE: `WAVE-NX-DOCSTUDIO-LIST-TEMPLATES-CLEAN.md` · Чеклист: `DOCSTUDIO-LIST-TEMPLATES-CLEAN-CHECKLIST.md` · SHA: `c9f687e4`→`eaa2a1ac` |
