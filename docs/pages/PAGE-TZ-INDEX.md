...
| `/proposals` | **proposals.page.md** | **FORMS-316 READY**; **SALES-313 DONE** family; **NX S20** «В студии»; **S37 DONE** convert accepted → заказ; **S42 DONE** variant-строки скрыты + badge «Семья» на master; **S43 DONE** expand семьи (варианты по фирмам, lazy org-name, loading/error, stale-guard); **S44 DONE** «Несколько фирм» attach-orgs → варианты (dialog multi-select org + markup %, POST + refresh, тосты); **S45 DONE** синхронизация состава вариантов с master (AlertDialog confirm → syncFromMaster → refresh + тосты); **S46 DONE** «В студии» на варианте семьи (открытие студии с id варианта / linked studio doc); **S47 DONE** «В заказ» только master/solo (variant guarded в template + method) |
