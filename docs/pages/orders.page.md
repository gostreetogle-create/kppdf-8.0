...
**TZ-NX-ORDER-WS-DOCS-CHIPS DONE (2026-09-15) — WAVE COMPLETE:** Workflow-chips nav (`OrderWsWorkflowChipsComponent`, ported from `home.page.ts`'s `WORKFLOW_CHIPS` — Главная/КП/Гант/Снабжение/Отгрузка, current = Заказ). Документы section (`OrderWsDocumentsComponent`): deep-link ported verbatim from the hub tray's existing «Документы» block (`/doc-constructor/templates?source=order&sourceId=`); honest «Нет связанных документов» — `PiStudioDocumentsService.list()` takes zero params (no order/source filter exists server-side), so no related-docs list is fetched rather than faking one from an unfiltered fetch. No `setTools`: no genuinely live, non-redundant tool-rail action was found beyond the header's plain links — rails stay history-only (the shell's own default for a page that registers nothing).
