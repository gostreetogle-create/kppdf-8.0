...
- **372 (snapshot edit/catalog resolution):** `Наименование / Описание / Артикул / Ед.` для source-linked Product редактируются только в snapshot конкретного КП; autosave не мутирует Product. При выходе из «Редактора таблицы» pending rows проходят единый review с решениями «Только в КП» / «Обновить изделие» / «Создать копию»; source update отправляет только identity fields с `expectedVersion`, Product copy rebinds текущую строку, а явное «Дублировать строку КП» сохраняет тот же Product. Коммерческие qty/price/discount/optional и row presentation никогда не sync в каталог.
