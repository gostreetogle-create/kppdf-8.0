...
- **373 (table font size):** `sheetLayout.tableFontSize` (px, default **12**, clamp **8…20**). UI: Параметры→«Вид листа» — «Шрифт таблицы»; тулбар редактора таблицы — «Шрифт» (тот же binding). Preview/build HTML таблицы получает `font-size` через `TablePhotoOptions`; старые КП без поля → 12. Не путать с `/doc-constructor/tables` и per-column font.
