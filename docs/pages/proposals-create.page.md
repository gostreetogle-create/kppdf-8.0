...
- Left rail: **Шаблон** + **Товары** + **Получатель**; Right: **Параметры** · **Редактор таблицы** · **Условия** (взаимоисключающие overlay-инструменты; порядок рейла TZ-SALES-356)
...
- 347/352: верхняя строка показывает статус «Черновик/Отправлено/Принято/Отклонено/В заказе/Отменено» и выбор разрешённого следующего статуса. «Сохранить версию» использует `freeze`, «Версии (N)» открывает сохранённый HTML только для просмотра без PATCH/autosave. «Создать заказ» всегда объясняет доступность: кнопка активна только для «Принято», иначе disabled с подсказкой «Сначала переведите КП в статус «Принято»». «Копировать КП» открывает duplicate в студии.
...
- **362 (WAVE-KP-STUDIO-CHROME):** flyout тиры **S** (`--kp-flyout-s` ≈20rem: Шаблон/Параметры/Условия) и **L** (`--kp-flyout-l` =794px: Товары/Редактор таблицы/Получатель); иконка Условий = `ScrollText`, не дубль `FileText` шаблона. Аудит: [`2026-08-12-kp-studio-flyout-chrome-audit.md`](../audits/2026-08-12-kp-studio-flyout-chrome-audit.md).
