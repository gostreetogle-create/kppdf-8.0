...
**«Документ изменён в другом месте» перестал спамить (`TZ-NX-DOCSTUDIO-REVISION-RACE-UX`, 2026-09-14):** ADD-PAGE-WRITE-SERIAL выше закрыл только часть self-race путей — layout-save (drag, свой параллельный `layoutSavePromise`, отдельный от `catalogWriteChain`), create table/text/image, `rehydrateLiveRowsAfterColumnChange`, смена issuer/doc-type/context (Кому/Ещё/Связи), rename и toggle-orientation всё ещё писали `expectedRevision` мимо очереди — PO: диалог «постоянно вылезает» в обычной работе, не только во второй вкладке. Теперь ВСЕ revision-gated записи документа идут через `catalogWriteChain`/`enqueueDocumentWrite` (layout-save обёрнут в `enqueueLayoutSave`, сохраняя внешний контракт `schedule()`/`flushLayouts()` без изменений). Диалог открывается **только** на настоящий revision-conflict (`isRevisionConflict()`: HTTP 409 + опциональный код `STUDIO_DOCUMENT_REVISION_CONFLICT`) — любой другой `!ok` (валидация, сеть, `template-blocks` PATCH стиля/контента/locked/visible — эти не gate revision вообще) теперь `toast.error`, не «другая вкладка». При 409 на очередной записи — один soft-retry (`attemptWithRevisionRetry`): `getById` → `document.set` → повтор той же операции со свежей `expectedRevision` (layout-save и drag-then-commit пересобирают payload из **текущих** `this.blocks()` на retry, не из снапшота до сети) — реальный второй писатель, у которого повтор тоже 409-ит, показывает диалог как и раньше, ровно один раз.
...
**Фото в ячейке таблицы (S48):** колонка с ключом-алиасом `photo` (тот же список, что в S47: `photo`/`image`/`рисунок`/`photourl`/`photoid`/`photo_id`/`photoids`/`photo_ids`/`фото`) рендерится как `<img>` при непустом значении ячейки, а не сырой текст/qty — на холсте (`studio-blocks-canvas.component.ts` `isPhotoColumnAt`) и в backend preview/PDF (`studio-data-resolver.ts` `renderPhotoCellHtml`, mirror в legacy `table-template.service.ts`, без переписывания Create КП). Определение колонки — только по `column.key` (тип колонки `photo`/`image` в NX-схеме `StudioTableColumn` пока не существует, добавлять не требовалось — все сценарии PO-канона используют ключ). **Пустая/битая ячейка — пусто, без текста (`TZ-NX-DOCSTUDIO-TABLE-PHOTO-EMPTY-BLANK`, 2026-09-14):** S48's «Нет фото»-плейсхолдер (класс `pi-photo-empty`) был сознательным решением на момент S48 — PO это отменил: нет фото / файл не резолвится / `<img>` не загрузился клиентски → пустая ячейка (высота ряда — от общего `td`-паддинга, не от содержимого), браузерная иконка «битой» картинки тоже не показывается.
...
- **«Источник строк»** для уже-Insert'нутой catalog-таблицы: этап A (2026-09-13) сначала заменил голый enum-дубль на статус «Строки: {Изделия|Модули|…} из Выбрано (N)» + **[Обновить строки]** + **[Сменить…]**; PO решил, что это тоже дубль (`TZ-NX-DOCSTUDIO-TABLE-ROWS-SOURCE-CLEANUP`, 2026-09-14) — **весь блок убран**, для wired catalog-таблицы в Свойствах не показывается ни статус, ни select вообще (`onCatalogSelectionChange` уже сам `putDataSet`'ит на каждое изменение «Выбрано», без ручного «Обновить»; смена вида = новый Insert или удалить слой, не смена enum на живой таблице). Канон: **строки каталога = Выбрано + Insert**, без «Обновить»/«Сменить» в Свойствах. Для `manual`/КП/заказа — как раньше, сразу полный select (единственный путь туда).
...
**Ширина колонок «По заголовкам» (`TZ-NX-DOCSTUDIO-TABLE-WIDTH-BY-HEADER`, 2026-09-14):** кнопка рядом с «+ Колонка» в Свойствах — `fitColumnWidthsByHeader()` (`studio-table-defaults.ts`) пересчитывает веса всех колонок по длине подписи (не по содержимому ячеек — дорого и дёргано на холсте); `photo`-alias жёстко узкий (кап 4), `name` шире подписи в 1.4×, `description` в 1.2× — остальные строго по длине label. Тот же вес теперь применяется автоматически к одной только НОВОЙ колонке при quick-add / «+ Колонка» (существующие ручные ширины соседних колонок не трогает); открытие старого документа без клика ширины не меняет. После кнопки/quick-add веса как обычно нормализуются в % через `columnWidthPercents` — ручная правка одного % после этого сохраняется как раньше.
...
**Библиотека текстов — категория → подкатегория (2026-09-11, `TZ-NX-TEXT-PICKER-FORM`):** `studio-text-properties.component.ts` — фильтр «Из библиотеки» теперь двухуровневый: select «Категория» (только корневые) → select «Подкатегория» (дети выбранного корня) → список названий, отфильтрованный по выбранной подкатегории (`TextBlock.categoryId` — всегда лист). «Все категории»/«Все подкатегории» показывают все блоки без фильтра — как раньше. Диалог сохранения в библиотеку (`TextBlockFormDialogComponent`, тот же для create/edit/save-to-library) требует явную подкатегорию (`Validators.required`); `slug` больше не поле формы — сервер генерирует его из имени. Category/subcategory select — kit `app-pi-select-add-row` с inline-create («+» → `TextBlockCategoryFormDialogComponent`, `TZ-NX-TEXT-BLOCK-CATEGORY-INLINE-CREATE`, 2026-09-14), без ухода на `/registries/text-block-categories`.
...
**Массовое удаление (`TZ-NX-DOCSTUDIO-LIST-BULK-DELETE`, 2026-09-14):** чекбокс на каждой строке + «Выбрать все» в шапке списка (tri-state — считает только видимые/отфильтрованные строки). При выборе ≥1 появляется панель «Удалить выбранное (N)» — тот же destructive-confirm диалог, что у одиночного «×»; подтверждение удаляет все выбранные документы параллельными вызовами существующего `DELETE /studio-documents/:id` (нового bulk-endpoint нет — soft-delete уже атомарен на один документ), список перезагружается, выбор сбрасывается. Частичный сетевой отказ не роняет остальные удаления — честный toast со счётом неудачных. Только `/studio` (список документов); `/studio/templates` не тронут.
...
**Fixed (`TZ-NX-DOCSTUDIO-UNSCOPED-ORG-SCOPE`, 2026-09-14):** снят self-lockout из known_limitation выше. `StudioDocumentService.findById`/`findAll` больше не резолвят unscoped-вызывающего (admin, `organizationId` null/undefined) к «первая org по имени» для scope-проверки — тот scope check теперь применяется только к caller'у, привязанному к реальной org (IDOR для bound-пользователя не снят: чужой org doc — всё ещё 403). Unscoped admin после смены Исполнителя сразу видит документ в `findAll` (не только fallback-org) и делает `GET`/`PATCH`/`+ Фото` без 403. FE: 403 со scope-текстом теперь honest toast «Нет доступа к документу этой фирмы…» (`isOrgScopeForbidden` в `studio-editor.page.ts`), не «Документ изменён в другом месте» (та проверка — только на 409) и не сырой английский текст бэкенда.
...
- **+ Текст** (`TZ-NX-DOCSTUDIO-TEXT-LIBRARY-INSERT-ON-ADD`, 2026-09-14): открывает picker — «Пустой текст» **или** библиотека (Категория → Подкатегория → список, тот же контракт, что «Из библиотеки» в Свойствах, `StudioTextLibraryPickerDialogComponent`). Выбор из библиотеки вставляет её `content` (+ имя как заголовок слоя); «Пустой текст» — прежнее поведение без изменений. Оба пути идут в активный текстовый слой, если он **пустой** текстовый, иначе — новый слой (анти-клобер: непустой активный текст не перезаписывается молча). Сохранение в библиотеку — по-прежнему из Свойств (`TextBlockFormDialogComponent`) или реестра «Тексты»; picker только читает.
...
**Вставить «Клиент»/«Поставщик»/«Плательщик» текстом (`TZ-NX-DOCSTUDIO-SELECTED-INSERT-PARTY-TEXT`, 2026-09-14):** рядом с «Вставить таблицу «…»» в том же `.insert-suggest` — по одной CTA на каждый заполненный anchor-чип (не на все три роли всегда: чипа нет — кнопки нет, как и у catalog-таргетов). Клик создаёт **новый текстовый слой** (`createTextLayer`, тот же revision-gated write path, что «+ Текст») с готовыми токенами — Клиент: `{{counterparty.name}}` + `ИНН {{counterparty.inn}}`; Поставщик: `{{anchor.supplier.name}}` + inn; Плательщик: `{{anchor.payer.name}}` — и сразу открывает Свойства текста для правки формулировки вокруг токенов. Раньше «якорь выбран, но товаров нет» показывало disabled «Вставить на лист» с подсказкой «Выберите товары» — честная party-кнопка теперь предлагается вместо неё; disabled-плейсхолдер остаётся только когда буфер пуст совсем. Не путать с «Изменить» (jump в Данные, выше) — это создание нового блока на листе, не редактирование существующего выбора.
...
- **Токены / Значения** (верх панели, всегда видно, `TZ-NX-DOCSTUDIO-TOKEN-EDITOR-CHIP`, 2026-09-14): сессионный переключатель отображения холста — default **«Значения»** (подставленное превью «как будет на бланке», `TZ-NX-DOCSTUDIO-TEXT-PROPS-CANON`, 2026-09-14); «Токены» — чипы, см. §2.2.
- **Одно выравнивание** (`TZ-NX-DOCSTUDIO-TEXT-PROPS-CANON`, 2026-09-14): SoT — `block.style.align` (ряд L/C/R/J в компактном блоке «Оформление» вместе с размером и цветом, сразу под RTE). TipTap-toolbar'ный align (`pi-rich-text-editor.component.ts`) скрыт в `[compact]="true"` — раньше это был второй, независимый путь (правил `textAlign` внутри HTML абзаца, но canvas/PDF читают только `block.style.align`), два контрола «выровнять» на одном экране. Не-compact usage (библиотека текстов, `text-block-form-dialog.component.ts`) TipTap-align сохраняет — там нет отдельного block-style контрола.
- Rich-text (TipTap): B/I/U inline-марки на уровне HTML; шрифт/межстрочный/размер/цвет/выравнивание — на уровне блока (`TemplateBlock.style`), один компактный блок «Оформление».
