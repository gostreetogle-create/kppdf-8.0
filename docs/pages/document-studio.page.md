...
**Имена для PO:** вкладка **«Докум.»** = модуль **Документ-студия** (`/studio`). Левая панель подбора ERP-контекста = **«Данные»**. IA-реформа панели: [`../audits/2026-09-05-docstudio-data-panel-ia-audit.md`](../audits/2026-09-05-docstudio-data-panel-ia-audit.md) · WAVE `WAVE-DOCSTUDIO-DATA-IA.md`.
...
| **Данные** | Wide flyout | TOC **Товары \| Выбрано \| Кому \| Связи \| Ещё** (D50, § 3.3) — витрина каталога, буфер выбранного + «Вставить на лист», Клиент/Плательщик, КП/Статус/Заказ, Поставщик/Исполнитель → PATCH `document.context` |
...
**IA (D50–D53, WAVE-DOCSTUDIO-DATA-IA):** канон сценария и словарь для менеджера —
[`../audits/2026-09-05-docstudio-data-panel-ia-audit.md`](../audits/2026-09-05-docstudio-data-panel-ia-audit.md).
Панель внутри — TOC из 5 категорий (`activeCategory`, дефолт **Товары**), не пять новых
chrome-иконок:

| TOC | Содержимое |
|-----|------------|
| **Товары** | Витрина каталога (Изделия/Модули/Детали/Материалы) — как раньше, S15/S27/S43 |
| **Выбрано** | Буфер: пусто → muted «Ничего не выбрано…»; иначе anchors + catalog chips + CTA «Вставить на лист» (D52, только совместимые kind'ы; пустой каталог → disabled + hint). TOC-badge = число позиций (party-якоря + Σ catalog count, не число групп) |
| **Кому** | **Клиент** первым; **Плательщик** — secondary disclosure «Указать плательщика отдельно» (открыт сразу, если уже задан) |
| **Связи** | КП, Статус КП (если `showKpStatus`), Заказ + строка-подсказка «подставятся их номер и строки» |
...
PATCH документа `{ context: { counterpartyId, quotationId, orderId, anchors, catalogSelections } }` с revision gate. Выбор КП/заказа заполняет клиента, если он пуст; удаление chip снимает позиции витрины и синхронизирует таблицы. Списки КП/заказов/контрагентов — live API при открытии редактора.
