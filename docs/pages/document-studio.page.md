...
|------------|-------------|--------|
| **Данные** | Элементы · Слои · Страницы · Свойства · Шаблон | Режимы + вывод только |

Сейчас: лево = 4 секции; ribbon = geometry controls (перенос в S17).
...
S16 целевая IA: слева остаётся только «Данные»; справа расположены «Элементы», «Слои», «Страницы», «Свойства» и «Шаблон». Все layout-панели открываются справа, а Data остаётся wide-панелью слева.
...
| **Данные** | Wide flyout | Витрина каталога (Изделия/Модули/Детали/Материалы), поиск и карточки `PiShowcaseCard md`; Исполнитель, Клиент, Плательщик, Поставщик, КП, Заказ → PATCH `document.context` |
...
S15: витрина каталога объединена с панелью «Данные». Выбор карточек сразу сохраняется в `context.catalogSelections`; если на листе одна ручная таблица, она автоматически получает соответствующий `catalog-*` источник и обновляет live-строки без перехода в Preview.
