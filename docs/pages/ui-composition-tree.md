...
13. **Карандаш (TZ-ORDERS-337 + TZ-QA-445F):** справа на каждой строке кнопка
    ...
    (как BOM `bom-edit`). Клик по остальной площади строки — только выбрать /
    раскрыть / свернуть; **не** открывает редактор и **не** уводит в карточку
    каталога (стол: из tray не navigates). Материал — карандаш / выбор.
