...
Тёмный TOC **КП | Договоры | Заказы** (`PiGroupWorkspaceComponent`, `DEALS_TOC_CHIPS`, активен **КП**); chip **КП** ведёт в `/proposals`. **Договоры** — disabled (id зарезервирован, страница ждёт D4). Жёлтый ряд пуст: существующая кнопка «Создать в студии» остаётся единственным CTA, без нового gold-ряда (TZ-NX-DEALS-D1).
