...
> **Назначение:** Единый формат описания страницы.
> Каждый файл в `docs/pages/<name>.page.md` описывает одну page-компоненту.
> При редактировании страницы — читай этот файл ПЕРЕД внесением изменений.
...
### Couplings (если поле читают ≥2 экрана)

Канон: [`docs/COUPLING-MAP.md`](../COUPLING-MAP.md). Не выдумывать локальный «активный».

| Поле | Этот экран | Другие экраны | Смысл |
|------|------------|---------------|-------|
| `Order.status` | колонка / фильтр | … | значение = тот же канон на всех экранах |
