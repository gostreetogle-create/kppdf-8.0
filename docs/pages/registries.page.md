...
| `modules` | Модули | `api` (`GET/POST/PATCH/DELETE /modules`, composition) | toolbar create; row: Редактировать, Открыть состав (dialog), Архивировать; module dialog also edits separate `workTypes[]` planning links; категория — обязательный select `type=module` (`TZ-NX-REG-CATEGORY-WIRE-MODULES`), список получил колонку «Категория» |
...
- **TZ-NX-REG-CATEGORY-WIRE-MODULES (2026-09-11)** — `ProductModule` gained a `categoryId` field (backend schema/DTO/service, `type=module` select in `ModuleFormDialogComponent`, required — no existing field to migrate, unlike Material/Product). Backend `findAll`/`findById` now `.populate('categoryId')`; the `modules` registry list column shows the resolved category name, not a raw ObjectId.
