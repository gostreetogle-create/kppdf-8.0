...
- **WAVE-GANTT-DARK / TZ-NX-GANTT-UNASSIGNED-AUTOEXPAND:** в режиме «По рабочим» группа «Не назначен» автоматически раскрывается при входе или появлении unassigned-работ; именованные рабочие остаются свёрнутыми. Баннер явно сообщает о раскрытой группе и сохраняет ссылку «Люди».
- **TZ-NX-GANTT-WORKERS-EMPTY-STATE:** workers-only без назначенных рабочих показывает честную подсказку с маршрутом в «Люди», а календарный wash тянется на доступную высоту даже при одной строке; fake worker rows и fit/zoom не добавляются.
