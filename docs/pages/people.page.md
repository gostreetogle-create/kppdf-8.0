...
**Цех** и является источником правды для списка кандидатов на Ганте; навыки
не назначают человека на конкретный заказ автоматически.
...
кандидатов `workTypeId → Person[]` по навыкам. Назначение на конкретный заказ
хранится отдельно в `Order.estimateWorkerOverrides[]`; без этой строки человек
не появляется на баре как назначенный. Баннер «Без исполнителя» и work-detail
ссылаются на `/registries/workers` и не приводят к 404.
