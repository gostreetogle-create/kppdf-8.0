...
- Добавление доступно администраторам и менеджерам; при ошибке API страница показывает русское уведомление через `extractErrorMessage`.
- Кнопка `✎` открывает небольшой диалог редактирования названия, символа и категории; ключ остаётся неизменным и отображается только для чтения.
- После успешного PATCH список перезагружается. Системные единицы можно редактировать по тем же правилам, но удаление отключено в UI и дополнительно защищено backend.
- Обычный пользователь сохраняет доступ к чтению, но не к мутациям.
