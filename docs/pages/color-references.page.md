...
## RAL input UX (TZ-DICT-318)

- В create/edit-диалоге код вводится отдельным полем только цифрами; слева постоянно виден readonly-префикс `RAL`.
- Подсказка: «Введите номер RAL — префикс подставится сам». Необязательное название добавляется как `RAL 9003 — Сигнальный белый`; без него сохраняется `RAL 9003`.
- При редактировании имена вида `RAL 9010 — Белый` разбираются обратно на код и название. Не-RAL имена остаются в обычном поле и не преобразуются.
- Старый неиспользуемый `color-references-form-dialog.component.ts` удалён после проверки импортов: активный путь использует singular `color-reference-form-dialog.component.ts`.
