...
Это давало несогласованный UX (одна форма показывает «Email неверный», другая «Неверный формат email»), и любое новое правило валидации нужно было добавлять в ~10–12 edit-диалогах руками (Audit 2 насчитал 16 файлов с FormBuilder/FormGroup/FormControl, из них ~10 — это edit-диалоги с inline `errorFor()`, остальные — filter/search формы на listing-страницах).
...
- **Зарезервированные ключи (`ruPhone`, `ruInn`)**: эти тексты уже определены в helper, но НИ ОДИН валидатор их ещё не производит. Когда добавите custom validator с именем `ruPhone` или `ruInn` (например, `Validators.pattern(/^\+7 .../): 'ruPhone'`), текст сразу подхватится.
