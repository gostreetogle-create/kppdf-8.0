...
- [x] MCP product passport category/status — **TZD-43**: product proposals accept optional `categoryId` and DTO status whitelist (`new|active|archived|draft`), journal mapping preserves both, and domain/validate/docs expose the contract; no category API or frontend changes ✅
