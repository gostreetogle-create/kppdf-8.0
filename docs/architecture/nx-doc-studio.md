...
### Что показали замеры legacy (DOCPLAT-01, 2026-08-30)

Живые числа из браузера 1440×900: `docs/agent-checklists/evidence/TZ-NX-DOCPLAT-01/_geometry.json`.

- **Инвариантность листа подтверждена, Δ = 0.** Книжный: 510.5×722 при открытой и закрытой панели, `right` 1343 в
  обоих случаях. Альбомный: 1260×730, `right` 1343 в обоих. Панель ровно 480px и уезжает в `x: -401`, не сужая лист.
  Это единственная часть закона, доказанная числами, — её и копируем.
- **Но альбомный лист — не A4.** 1260/730 = **1.726** против 1.414 у A4; книжный при этом идеален (0.7069 против
  0.7071). То есть в альбомной ориентации оператор видит бумагу на ~22% шире, чем она есть, и блок, поставленный на
  глаз, в PDF окажется не там. Width-first при `w: 1260` требует `h: 891`, что в доступные 730 не влезает, значит
  правило обязано было откатиться к height-first и дать `w ≈ 1032`. **S2 копирует инвариантность, но не пропорцию:**
  лист сохраняет соотношение A4 всегда, `fit` выбирается по той стороне, которая упирается первой.
- **Клик по листу не сворачивает панель** (дефект D7): в legacy панель закрывается только повторным кликом по иконке
  рельса, хотя п.7 закона требует клика по плоскости. Значит в S2 это надо **реализовать**, а не «сохранить как есть»
  — и только после этого плавающему тулбару типографики понадобится `stopPropagation`.
- `stage` в замерах не снялся (`null`), поэтому правило «≈8px справа» числами **не доказано**: измерено только
  расстояние до края окна (97px при рельсе 4rem). S2 обязан снять прямоугольник плоскости и доказать зазор.
