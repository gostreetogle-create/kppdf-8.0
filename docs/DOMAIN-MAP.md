...
| **Sales / КП / Orders** | `quotation`, `order`, `contract`, `shipment`, `invoice`, `order-closing` | `/proposals`, `/proposals/create`, `/orders`, `/orders/:id`, `/contracts`, `/shipping` | `proposals`, `proposals-create`, `orders`, `contracts`, `shipping` (stub) | `docs/audits/2026-08-08-sales-to-shop-flow-canon.md`, `docs/data-model.md` | **КП (`quotation`/proposals UI) ≠ Order (цех)**; `isStub` КП ≠ реальное; договор ≠ заказ |
