...
|-------|-----------|-----------|---------|-------------|-----------|
| **Auth / Users / Roles** | `auth`, `user`, `role`, `permissions`, `rate-limit` | `/login`, `/admin/users`, `/admin/roles` | `login.page.md`; admin — **NO** | `docs/RBAC-CONTRACT.md`, `permissions.constants.ts` | `User` (аккаунт) ≠ `Worker` (сотрудник цеха); `user:read` ≠ `user:admin` |
...
| **Sales / КП / Orders** | `quotation`, `order`, `contract`, `shipment`, `invoice`, `order-closing` | `/proposals`, `/proposals/create`, `/orders`, `/orders/:id`, `/contracts`, `/shipping` | `proposals`, `proposals-create`, `orders`, `contracts`; shipping — **NO** | `docs/audits/2026-08-08-sales-to-shop-flow-canon.md`, `docs/data-model.md` | **КП (`quotation`/proposals UI) ≠ Order (цех)**; `isStub` КП ≠ реальное; договор ≠ заказ |
...
| **Production** | `production-order`, `work-order`, `work-order-operation`, `work-type`, `work-center`, `routing-step`, `tech-process`, `order-task` | `/production`, `/work-types` | `production-cockpit`, `work-types` | `docs/SECTION-READINESS.md` (Производство SHELL/MVP) | `WorkType` (каталог) ≠ `RoutingStep` (шаг маршрута); `production-order` ≠ sales `order` |
...
| **Admin / Settings** | `admin`, `setting`, `feature-flag`, `form-profiles`, `counter`, `site` | `/admin/*`, `/dictionaries/form-profiles` | `form-profiles`; admin — **NO** | `docs/RBAC-CONTRACT.md`, `permissions.constants.ts` | FE `admin/*` routes ≠ BE `admin` module; form-profiles ≠ роли |
...
| `/inventory` | yes — `inventory-dashboard` | ⚠️ README пишет `/dashboard` — drift (P2) |
