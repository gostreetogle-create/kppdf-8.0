...
> Author: senior-orchestrator В· TZ-254 В· 2026-07-31 В· Status: CANONICAL
...
- `<section>` is one of the named entities in the catalog (see В§6).
- `<action>` is one of `read`, `write`, `admin` (see В§2).
- The wildcard character `*` (see В§4) is the single exception to this
  two-token shape.
...
> Peer-audit 2026-08-02 Finding 5 вЂ” СЃРј. С‚Р°РєР¶Рµ `TZ-RBAC-304`,
...
`createdBy`) or **shared** (corporate data вЂ” section-level permission is
...
- `User.schema` (rename `role: string` в†’ `roleIds: ObjectId[]`)
- `RoleService.findOrCreateManyForUser()`
- Auth layer (`JWT payload role` в†’ `JWT payload roles[]`)
- `effectivePermissions` (loop over all roles instead of one)

Markers: do not introduce `pendingRoleIds[]` as a stop-gap вЂ” that
...
call. **Use sparingly** вЂ” every `*` should be paired with a
...
- Adding a new section в†’ append PERMISSIONS entry + update `Section` union.
- Removing a permission key в†’ must update ALL role seeds AND every
  `@Permissions(...)` site that references it (TZ-255 audit sweep).
- Renaming a section в†’ coordinate with TZ-255 + TZ-256 in a paired
  PR; requires user-data audit if any role already references the old
  key.

Authored: 2026-07-31 В· TZ-254 implementation pass. Verified against
