...
- ~~Документ «КП 11.09.2026 (2)» → блок «Продукты» — старые данные могут визуально не совпадать с 6/7 колонками~~ — **CLOSEOUT (TZ-NX-DOCSTUDIO-STALE-LIVEROWS-HEAL, `<SHA>`)**: побочная находка выше пофикшена авто-heal при открытии/загрузке документа — детект рассинхрона `liveRows`/колонок теперь срабатывает не только по факту редактирования структуры колонок, но и на каждом load, для live-source И manual/no-`dataSource` таблиц (сам этот документ — второй случай: у блока «Продукты» `dataSource` вообще отсутствует). **PO больше не обязан пересохранять** «КП 11.09.2026 (2)» руками — таблица выравнивается сама при следующем открытии.
