...
- Документ «КП 11.09.2026 (2)» → блок «Продукты» — старые данные могут визуально не совпадать с 6/7 колонками (см. «Побочная находка» выше). Не баг кода, просьба один раз пересохранить строки/источник в этом конкретном документе, если будет странно выглядеть.

## SHA

Нет продуктовых фиксов в этой волне — 0 FAIL на всех gates и всех 18 smoke-проверках. Единственный коммит — сам smoke-скрипт + этот аудит + evidence (см. финальный commit hash в Executor report).
