...
задача при необходимости. SHA `9e79911d`.

## 7. Closeout 03/3 (2026-09-12) — `TZD-AI-IMPORT-MAPPING-OLLAMA` DONE — WAVE COMPLETE

`suggestWithAi` больше не читает `aiState.port` из замыкания — провайдер
теперь параметр (`{baseUrl, apiKey?, model}`, тот же приём, что
`normalizeStep` из TZ-01). `suggestMapping()` пробует: встроенный раннер →
**новое: `resolveProvider`/`pingProvider` (Ollama/remote из
`config.aiProvider`)** → MCP-классификатор → детерминированный
`classifyHeaders` (был всегда доступен, не тронут). `pingProvider` из
постоянного stub (`return false`) стал реальной проверкой: `local-ollama` —
`GET /api/tags` с коротким таймаутом; `remote` — оптимистично `true` (нет
универсального дешёвого пинга для произвольного OpenAI-совместимого
шлюза, реальный сбой честно всплывает из `chatCompletion`). Сообщение
после подбора называет источник (`AI: Ollama` / `удалённый провайдер` /
`встроенная модель`), не выдаёт детерминированный путь за AI-успех.

**WAVE-DESKTOP-AI-IMPORT-BASELINE — все 3 задачи DONE.** SHA: 01
`0a4bbe76` · 02 `9e79911d` · 03 (см. checklist после push). Baseline
записан (`parse_ok=88.1%` на 59 синтетических fixtures, выше Soup-порога
`≥75%`); опт-ин датасет-лог включаем оператором, пока 0 строк накоплено —
Soup остаётся **PARK** до накопления `≥300` строк и/или прогона против
реальной модели (Ollama не запущена в этой среде за всю волну — ни один
прогон не претендует на live-model accuracy, везде явно задокументировано).
