...
## Closeout 05/5 (2026-09-11) — `TZ-NX-DOCSTUDIO-TABLE-KINDS-DISCOVER` DONE — WAVE COMPLETE

Слабая текстовая подсказка «Реестр видов — «Реестры → Документы → Виды
таблиц»» заменена на настоящую кликабельную ссылку (`routerLink`, новая
вкладка — редактирование документа не прерывается S38-guard'ом, который
живёт выше по дереву и не стоило тащить в этот leaf-компонент только ради
одной ссылки). Optional-пункт (seed/dedupe «Продукты») оказался НЕ
косметическим: все 3 дубликата, найденные в TZ-04, не имели колонки
«Количество» вовсе — то есть даже после разлока `columnsEditable()` в TZ-02
у оператора не было бы что показывать/скрывать, поле просто не
существовало в сохранённой структуре. Написана и **реально прогнана
дважды** против локальной dev-БД идемпотентная миграция (тот же паттерн,
что `2026-08-02-TZ-DOC-307-backfill-template-categories.ts`): старейший
активный «Продукты» — канонический, 2 дубликата деактивированы (не
удалены), колонка «Количество» добавлена. Второй прогон — 0 изменений,
идемпотентность подтверждена не только моками, но и на реальных данных.

**WAVE-NX-DOCSTUDIO-TABLE-PROPS — все 5 задач DONE.** SHA: 01 `0df45baf` ·
02 `24e2ae14` · 03 `d042de05` · 04 `7d904839` · 05 см. checklist после push.
Итог волны: свойства таблицы шире (820px, только для таблиц); reorder/add
колонок разлочен независимо от вида/источника строк; qty для catalog-строк
редактируема и переживает refresh (override на блоке, не на Product); фото
в live-строках — реально проверяется на диске, не просто на наличие
DB-ссылки; discoverability реестра видов + очищен канонический «Продукты».
