...
export class ProductController {
  ...
  @Get() @Roles('admin', 'manager') @ApiOperation({ summary: 'List all products with pagination and filters' }) @ApiQuery({ name: 'page', required: false }) @ApiQuery({ name: 'limit', required: false }) @ApiQuery({ name: 'search', required: false }) @ApiQuery({ name: 'categoryId', required: false }) @ApiQuery({ name: 'status', required: false }) @ApiQuery({ name: 'isActive', required: false }) @ApiQuery({ name: 'sortBy', required: false }) @ApiQuery({ name: 'sortOrder', required: false }) @ApiResponse({ status: 200, description: 'Paginated list of products' })
  list(@CurrentUser() user: AuthenticatedUser, @Query('page') page = '1', @Query('limit') limit = '20', @Query('search') search?: string, @Query('categoryId') categoryId?: string, @Query('status') status?: string, @Query('isActive') isActive?: string, @Query('sortBy') sortBy?: string, @Query('sortOrder') sortOrder?: 'asc' | 'desc') { return this.service.findAll({ page: parseInt(page, 10), limit: parseInt(limit, 10), search, categoryId, status, isActive: isActive === 'true' ? true : isActive === 'false' ? false : undefined, sortBy, sortOrder }, user.organizationId); }
  @Get(':id/where-used') @Roles('admin', 'manager', 'user') getWhereUsed(@Param('id') id: string, @Query('page') page = '1', @Query('limit') limit = '20', @CurrentUser() user: AuthenticatedUser) { return this.catalogGraph.getWhereUsed('product', id, { page: parseInt(page, 10), limit: parseInt(limit, 10), organizationId: user.organizationId }); }
  @Get(':id/composition') @Roles('admin', 'manager', 'user') getComposition(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser) { return this.service.getComposition(id, user.organizationId); }
  ...
  @Get(':id/tree') @Roles('admin', 'manager', 'user') getTree(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser, @Query('maxDepth') maxDepth?: string) { return this.service.getTree(id, maxDepth === undefined ? MAX_DEPTH : Number(maxDepth), user.organizationId); }
  @Get(':id') @Roles('admin', 'manager', 'user') findOne(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser) { return this.service.findById(id, user.organizationId); }
