 ...
 *
 * TZ-QA-445B — material field uses shared `pi-select-add-row` + MaterialFormDialog
 * (same write-path as catalog/supply). After inline create, materialId is selected
 * and unit from the card is shown next to quantity.
 */
@Component({
        ...
        <app-pi-form-section title="Основные данные" headingId="movement-sec-basics" tone="gold">
          ...
          >
            <app-pi-select-add-row
              ...
              addAriaLabel="Новый материал"
              addDataTest="mv-material-add"
              (addClick)="openCreateMaterial()"
            >
              <select
                id="mv-material"
                class="pi-input w-full"
...
export class StockMovementFormDialogComponent {
  ...
  /** Optimistically merged after inline MaterialFormDialog create (TZ-QA-445B). */
  private readonly createdMaterials = signal<MaterialOption[]>([]);
  ...
  protected readonly materials = computed(() => {
    const fromApi = this.materialsRes.value()?.items ?? [];
    const extras = this.createdMaterials();
    if (!extras.length) return fromApi;
    const seen = new Set(fromApi.map((m) => m._id));
    return [...fromApi, ...extras.filter((m) => !seen.has(m._id))];
  });
  ...
  protected readonly qtyLabel = computed(() => {
    const unit = this.selectedMaterialUnit();
