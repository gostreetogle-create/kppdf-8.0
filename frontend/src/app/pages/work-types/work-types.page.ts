                ...
                <td class="pi-cell align-top text-center">
                  ...
                  >
                    <span
                      ...
                      [class.translate-x-2]="row.isActive"
                      [class.-translate-x-2]="!row.isActive"
                    ></span>
                  </button>
                </td>
...
export class WorkTypesPage implements OnInit {
  ...
  protected onToggleActive(wt: WorkType): void {
    this.service.update(wt._id, { isActive: !wt.isActive }).subscribe((res) => {
      if (res.ok) {
        this.toast.success(wt.isActive ? `«${wt.name}» деактивирован` : `«${wt.name}» активирован`);
        this.listRes.reload();
      } else {
